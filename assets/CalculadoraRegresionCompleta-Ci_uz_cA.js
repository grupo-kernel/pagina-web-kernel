function ue(e,t){if(!Array.isArray(t)||t.length<3)throw new Error(`${e} debe contener al menos tres observaciones.`);const a=t.map(r=>r==null||typeof r=="string"&&r.trim()===""?NaN:Number(r));if(!a.every(Number.isFinite))throw new Error(`${e} contiene valores no numéricos o no finitos.`);return a}function fe(e){return e[0].map((t,a)=>e.map(r=>r[a]))}function re(e,t){return e.map(a=>t[0].map((r,i)=>a.reduce((c,n,l)=>c+n*t[l][i],0)))}function te(e,t){return e.map(a=>a.reduce((r,i,c)=>r+i*t[c],0))}function Me(e,t){const a=e.length,r=e[0].length,i=e.map(o=>[...o]),c=[...t],n=[];for(let o=0;o<r;o+=1){let d=0;for(let u=o;u<a;u+=1)d=Math.hypot(d,i[u][o]);const p=Number.EPSILON*Math.max(a,r)*Math.max(1,...n.map(Math.abs))*100;if(!(d>p))throw new Error("La matriz de predictores es singular. Elimine variables redundantes o perfectamente colineales.");const m=i[o][o]>=0?1:-1,v=Array.from({length:a-o},(u,$)=>i[o+$][o]);v[0]+=m*d;const h=v.reduce((u,$)=>u+$**2,0);if(!(h>0))throw new Error("La matriz de predictores es singular. Elimine variables redundantes o perfectamente colineales.");for(let u=o;u<r;u+=1){let $=0;for(let k=0;k<v.length;k+=1)$+=v[k]*i[o+k][u];const C=2*$/h;for(let k=0;k<v.length;k+=1)i[o+k][u]-=C*v[k]}let y=0;for(let u=0;u<v.length;u+=1)y+=v[u]*c[o+u];const g=2*y/h;for(let u=0;u<v.length;u+=1)c[o+u]-=g*v[u];n.push(i[o][o])}const l=Array(r).fill(0);for(let o=r-1;o>=0;o-=1){let d=c[o];for(let p=o+1;p<r;p+=1)d-=i[o][p]*l[p];l[o]=d/i[o][o]}const s=Array.from({length:r},()=>Array(r).fill(0));for(let o=0;o<r;o+=1)for(let d=r-1;d>=0;d-=1){let p=d===o?1:0;for(let m=d+1;m<r;m+=1)p-=i[d][m]*s[m][o];s[d][o]=p/i[d][d]}return{coeficientes:l,inversaProducto:re(s,fe(s))}}function ze(e,t){const a=e[0].length,r=e.map(s=>t?se(s):0),i=e.map((s,o)=>{const d=r[o],p=Math.sqrt(s.reduce((v,h)=>v+(h-d)**2,0)/a),m=Math.max(1,...s.map(Math.abs));if(!(p>Number.EPSILON*m*10))throw new Error(`El predictor X${o+1} no presenta variabilidad suficiente para estimar el modelo.`);return p}),c=Array.from({length:a},(s,o)=>[...t?[1]:[],...e.map((d,p)=>(d[o]-r[p])/i[p])]),n=c[0].length,l=Array.from({length:n},()=>Array(n).fill(0));return t?(l[0][0]=1,e.forEach((s,o)=>{l[0][o+1]=-r[o]/i[o],l[o+1][o+1]=1/i[o]})):e.forEach((s,o)=>{l[o][o]=1/i[o]}),{matriz:c,transformacion:l,medias:r,escalas:i}}function se(e){return e.reduce((t,a)=>t+a,0)/e.length}function je(e,t=se(e)){return e.reduce((a,r)=>a+(r-t)**2,0)}function _(e){const t=[676.5203681218851,-1259.1392167224028,771.3234287776531,-176.6150291621406,12.507343278686905,-.13857109526572012,9984369578019572e-21,15056327351493116e-23];if(e<.5)return Math.log(Math.PI)-Math.log(Math.sin(Math.PI*e))-_(1-e);let a=.9999999999998099;const r=e-1;t.forEach((c,n)=>{a+=c/(r+n+1)});const i=r+t.length-.5;return .5*Math.log(2*Math.PI)+(r+.5)*Math.log(i)-i+Math.log(a)}function pe(e,t,a){const n=e+t,l=e+1,s=e-1;let o=1,d=1-n*a/l;Math.abs(d)<1e-300&&(d=1e-300),d=1/d;let p=d;for(let m=1;m<=200;m+=1){const v=2*m;let h=m*(t-m)*a/((s+v)*(e+v));d=1+h*d,Math.abs(d)<1e-300&&(d=1e-300),o=1+h/o,Math.abs(o)<1e-300&&(o=1e-300),d=1/d,p*=d*o,h=-(e+m)*(n+m)*a/((e+v)*(l+v)),d=1+h*d,Math.abs(d)<1e-300&&(d=1e-300),o=1+h/o,Math.abs(o)<1e-300&&(o=1e-300),d=1/d;const y=d*o;if(p*=y,Math.abs(y-1)<3e-14)break}return p}function ne(e,t,a){if(e<=0)return 0;if(e>=1)return 1;const r=Math.exp(_(t+a)-_(t)-_(a)+t*Math.log(e)+a*Math.log(1-e));return e<(t+1)/(t+a+2)?r*pe(t,a,e)/t:1-r*pe(a,t,1-e)/a}function Ce(e,t){if(!Number.isFinite(e))return 0;const a=t/(t+e**2);return Math.min(1,ne(a,t/2,.5))}function qe(e,t,a){if(!Number.isFinite(e))return 0;if(e<=0)return 1;const r=a/(a+t*e);return ne(r,a/2,t/2)}function me(e,t){if(e===0)return .5;if(e===1/0)return 1;if(e===-1/0)return 0;const a=t/(t+e**2),r=.5*ne(a,t/2,.5);return e>0?1-r:r}function he(e,t){if(!(e>0&&e<1)||!(t>0))throw new Error("No fue posible calcular el cuantil de la distribución t.");if(e===.5)return 0;if(e<.5)return-he(1-e,t);let a=0,r=1;for(;me(r,t)<e&&r<1e12;)r*=2;for(let i=0;i<120;i+=1){const c=(a+r)/2;me(c,t)<e?a=c:r=c}return(a+r)/2}function Le(e,t){const a=e[0].length;return Array.from({length:a},(r,i)=>[...t?[1]:[],...e.map(c=>c[i])])}function Re(e,t){return e.length<2?[]:e.map((a,r)=>{const i=e.filter((c,n)=>n!==r);try{const c=ge({y:a,X:i,nombresPredictores:t.filter((l,s)=>s!==r),incluirIntercepto:!0,nivelConfianza:.95,calcularDiagnosticos:!1,calcularVif:!1}),n=Math.max(1-c.ajuste.r2,1e-12);return{variable:t[r],tolerancia:n,vif:1/n}}catch{return{variable:t[r],tolerancia:0,vif:1/0}}})}function ge({y:e,X:t,nombresPredictores:a=[],incluirIntercepto:r=!0,nivelConfianza:i=.95,calcularDiagnosticos:c=!0,calcularVif:n=!0}){const l=ue("La variable dependiente",e);if(!Array.isArray(t)||!t.length)throw new Error("Debe proporcionar al menos una variable predictora.");const s=t.map((x,b)=>ue(a[b]||`X${b+1}`,x));if(!s.every(x=>x.length===l.length))throw new Error("Todas las variables deben contener la misma cantidad de observaciones.");if(!(i>0&&i<1))throw new Error("El nivel de confianza debe estar entre 0 y 1.");const o=s.map((x,b)=>a[b]||`X${b+1}`),d=se(l),p=je(l,d),m=Math.max(1,...l.map(Math.abs));if(Math.sqrt(p/l.length)<=Number.EPSILON*m*10)throw new Error("La variable dependiente no presenta variabilidad; no es posible ajustar una regresión.");const v=Le(s,r),h=ze(s,r),y=l.length,g=v[0].length,u=y-g;if(u<=0)throw new Error("No hay suficientes observaciones para estimar el modelo.");const $=Me(h.matriz,l),C=te(h.transformacion,$.coeficientes),k=re(re(h.transformacion,$.inversaProducto),fe(h.transformacion));if(!C.every(Number.isFinite)||!k.flat().every(Number.isFinite))throw new Error("El ajuste produjo resultados no finitos. Revise la escala, colinealidad y valores extremos de los predictores.");const I=te(h.matriz,$.coeficientes),z=l.map((x,b)=>x-I[b]),D=r?p:l.reduce((x,b)=>x+b**2,0),V=z.reduce((x,b)=>x+b**2,0),W=Math.max(D-V,0),T=D>1e-12?1-V/D:1,ce=r?1-(1-T)*(y-1)/u:1-(1-T)*y/u,j=V/u,B=Math.sqrt(j),ve=z.reduce((x,b)=>x+Math.abs(b),0)/y,q=g-(r?1:0),G=q>0&&j>1e-12?W/q/j:1/0,R=q>0?qe(G,q,u):null,le=he(.5+i/2,u),Q=1-i,ye=[...r?["Intercepto"]:[],...o],$e=C.map((x,b)=>{const w=Math.sqrt(Math.max(j*k[b][b],0)),Y=w>1e-12?x/w:x===0?0:1/0,ee=Ce(Math.abs(Y),u);return{termino:ye[b],estimacion:x,errorEstandar:w,t:Y,p:ee,intervaloConfianza:{inferior:x-le*w,superior:x+le*w,nivel:i}}}),S=h.matriz.map(x=>{const b=te($.inversaProducto,x);return x.reduce((w,Y,ee)=>w+Y*b[ee],0)}),K=z.map((x,b)=>{const w=Math.sqrt(Math.max(j*(1-S[b]),1e-12));return x/w}),Z=z.map((x,b)=>{const w=S[b];return g>0&&j>1e-12?x**2/(g*j)*w/Math.max((1-w)**2,1e-12):0}),de=z.reduce((x,b)=>x+b**2,0),we=z.slice(1).reduce((x,b,w)=>x+(b-z[w])**2,0),ke=de>1e-12?we/de:null,O=n&&s.length>1?Re(s,o):[],J=c?Z.map((x,b)=>({observacion:b+1,distanciaCook:x,leverage:S[b],residuoEstandarizado:K[b]})).filter(x=>x.distanciaCook>4/y||Math.abs(x.residuoEstandarizado)>2||x.leverage>2*g/y):[],Ee=[`El modelo explica ${(T*100).toFixed(2)} % de la variabilidad de la variable dependiente.`,`El R² ajustado es ${ce.toFixed(4)} y el error estándar de estimación es ${B.toFixed(4)}.`,R===null?"No se calculó una prueba global del modelo.":R<Q?`El modelo global es estadísticamente significativo (F = ${G.toFixed(4)}, p ${R<.001?"< 0.001":`= ${R.toFixed(4)}`}).`:`El modelo global no alcanza significación estadística con α = ${Q.toFixed(3)} (F = ${G.toFixed(4)}, p = ${R.toFixed(4)}).`,O.some(x=>x.vif>=10)?"Se detectó multicolinealidad severa en al menos un predictor (VIF ≥ 10).":O.some(x=>x.vif>=5)?"Se detectó multicolinealidad moderada en al menos un predictor (VIF ≥ 5).":O.length?"No se detectó multicolinealidad relevante mediante el criterio VIF.":"El VIF no aplica a un modelo con un solo predictor.",J.length?`Se identificaron ${J.length} observaciones potencialmente influyentes que deben revisarse.`:"No se identificaron observaciones claramente influyentes mediante los criterios automáticos."];return{tipoModelo:s.length===1?"regresion-lineal-simple":"regresion-lineal-multiple",n:y,numeroPredictores:s.length,incluirIntercepto:r,nivelConfianza:i,alfa:Q,nombresPredictores:o,estabilizacion:{medias:h.medias,escalas:h.escalas,coeficientesEstandarizados:$.coeficientes},coeficientes:$e,ajuste:{r2:T,r2Ajustado:ce,mae:ve,mse:j,rmse:B,errorEstandarEstimacion:B},anova:{sumaCuadradosModelo:W,sumaCuadradosError:V,sumaCuadradosTotal:D,gradosLibertadModelo:q,gradosLibertadError:u,gradosLibertadTotal:r?y-1:y,mediaCuadraticaModelo:q>0?W/q:null,mediaCuadraticaError:j,f:G,p:R},predichos:I,residuos:z,diagnosticos:{leverage:S,residuosEstandarizados:K,distanciaCook:Z,durbinWatson:ke,vif:O,observacionesInfluyentes:J},datosGrafico:l.map((x,b)=>({observacion:b+1,observado:x,predicho:I[b],residuo:z[b],residuoEstandarizado:K[b],leverage:S[b],distanciaCook:Z[b],predictores:s.map(w=>w[b])})),interpretacion:Ee}}function Ae(e,t){if(!e?.coeficientes?.length)throw new Error("El modelo proporcionado no es válido.");if(!Array.isArray(t)||t.length!==e.numeroPredictores)throw new Error(`Debe proporcionar ${e.numeroPredictores} valores predictores.`);const a=t.map(c=>c==null||typeof c=="string"&&c.trim()===""?NaN:Number(c));if(!a.every(Number.isFinite))throw new Error("Los valores predictores deben ser numéricos.");if(e.estabilizacion?.medias?.length===e.numeroPredictores&&e.estabilizacion?.escalas?.length===e.numeroPredictores&&e.estabilizacion?.coeficientesEstandarizados?.length===e.coeficientes.length)return[...e.incluirIntercepto?[1]:[],...a.map((n,l)=>(n-e.estabilizacion.medias[l])/e.estabilizacion.escalas[l])].reduce((n,l,s)=>n+l*e.estabilizacion.coeficientesEstandarizados[s],0);const r=e.coeficientes.map(c=>c.estimacion);return[...e.incluirIntercepto?[1]:[],...a].reduce((c,n,l)=>c+n*r[l],0)}function E(e,t=3){return Number.isFinite(e)?new Intl.NumberFormat("es-DO",{maximumFractionDigits:t}).format(e):"—"}function oe(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function A(e,t=.08){const a=e.filter(Number.isFinite);let r=Math.min(...a),i=Math.max(...a);if(!a.length)return{minimo:0,maximo:1};if(Math.abs(i-r)<1e-12){const c=Math.max(Math.abs(i)*.1,1);r-=c,i+=c}else{const c=(i-r)*t;r-=c,i+=c}return{minimo:r,maximo:i}}function ie(e,t,a,r){const i=Math.max(t-e,1e-12);return c=>a+(c-e)/i*(r-a)}function U({ancho:e,alto:t,margen:a,rangoX:r,rangoY:i,etiquetaX:c,etiquetaY:n}){const l=[],s=[],o=[],d=ie(r.minimo,r.maximo,a.izquierdo,e-a.derecho),p=ie(i.minimo,i.maximo,t-a.inferior,a.superior);for(let m=0;m<=4;m+=1){const v=m/4,h=r.minimo+v*(r.maximo-r.minimo),y=i.minimo+v*(i.maximo-i.minimo),g=d(h),u=p(y);l.push(`
    ...
    `}function xe(e,t){const a=e.coeficientes.map(i=>i.estimacion);return[...e.incluirIntercepto?[1]:[],...t].reduce((i,c,n)=>i+c*a[n],0)}function Ne(e,t){if(e.numeroPredictores!==1)return"";const a=e.nombresPredictores[0]||"X",r=e.datosGrafico,i=r.map(g=>g.predictores[0]),c=r.map(g=>g.observado),n=A(i),l=A([...c,...r.map(g=>g.predicho)]),o=U({ancho:640,alto:360,margen:{izquierdo:66,derecho:28,superior:24,inferior:60},rangoX:n,rangoY:l,etiquetaX:a,etiquetaY:t}),d=Math.min(...i),p=Math.max(...i),m=xe(e,[d]),v=xe(e,[p]),h=r.map(g=>`
        <line
    ...
    `).join("");return N("Dispersión y recta ajustada","Los puntos representan los datos observados; la línea muestra el modelo estimado y los segmentos verticales representan los residuos.",P(`
            ${o.contenido}
            ${h}
            <line
                x1="${o.x(d)}"
    ...
    `).join("");return N("Observado frente a predicho","Un ajuste adecuado concentra los puntos cerca de la línea diagonal de igualdad.",P(`
            ${n.contenido}
            <line
                x1="${n.x(i.minimo)}"
                y1="${n.y(i.minimo)}"
            ...
            />
            ${l}
        `,"Gráfico de valores observados frente a valores predichos"))}function Ie(e){const t=e.datosGrafico.map(s=>s.predicho),a=e.datosGrafico.map(s=>s.residuo),r=A(t),i=A([...a,0]),n=U({ancho:640,alto:360,margen:{izquierdo:66,derecho:28,superior:24,inferior:60},rangoX:r,rangoY:i,etiquetaX:"Valor predicho",etiquetaY:"Residuo"}),l=e.datosGrafico.map((s,o)=>`
        <circle
            cx="${n.x(s.predicho)}"
            cy="${n.y(s.residuo)}"
            r="5"
    ...
    `).join("");return N("Residuos frente a valores predichos","Se espera una nube aproximadamente aleatoria alrededor de cero. Los puntos rojos tienen residuos estandarizados con magnitud mayor que 2.",P(`
            ${n.contenido}
            <line
                x1="${n.x(r.minimo)}"
                y1="${n.y(0)}"
            ...
            />
            ${l}
        `,"Gráfico de residuos frente a valores predichos"))}function Se(e){const t=e.residuos,a=t.length,r=Math.max(4,Math.ceil(Math.log2(a)+1)),i=Math.min(...t),c=Math.max(...t),l=Math.max(c-i,1e-9)/r,s=Array.from({length:r},(u,$)=>({inferior:i+$*l,superior:i+($+1)*l,frecuencia:0}));t.forEach(u=>{const $=Math.min(Math.floor((u-i)/l),r-1);s[$].frecuencia+=1});const o=Math.max(...s.map(u=>u.frecuencia),1),d=640,p=360,m={izquierdo:60,derecho:24,superior:24,inferior:68},v=d-m.izquierdo-m.derecho,h=p-m.superior-m.inferior,y=v/r,g=s.map((u,$)=>{const C=u.frecuencia/o*h,k=m.izquierdo+$*y,I=p-m.inferior-C;return`
            <rect
                x="${k+1}"
                y="${I}"
        ...
        `}).join("");return N("Histograma de residuos","Permite examinar visualmente la simetría, concentración y posibles colas de la distribución residual.",P(`
        ...
        `,"Histograma de los residuos"))}function Fe(e){const t=[-39.6968302866538,220.946098424521,-275.928510446969,138.357751867269,-30.6647980661472,2.50662827745924],a=[-54.4760987982241,161.585836858041,-155.698979859887,66.8013118877197,-13.2806815528857],r=[-.00778489400243029,-.322396458041136,-2.40075827716184,-2.54973253934373,4.37466414146497,2.93816398269878],i=[.00778469570904146,.32246712907004,2.445134137143,3.75440866190742],c=.02425,n=1-c;if(e<c){const o=Math.sqrt(-2*Math.log(e));return(((((r[0]*o+r[1])*o+r[2])*o+r[3])*o+r[4])*o+r[5])/((((i[0]*o+i[1])*o+i[2])*o+i[3])*o+1)}if(e>n){const o=Math.sqrt(-2*Math.log(1-e));return-(((((r[0]*o+r[1])*o+r[2])*o+r[3])*o+r[4])*o+r[5])/((((i[0]*o+i[1])*o+i[2])*o+i[3])*o+1)}const l=e-.5,s=l*l;return(((((t[0]*s+t[1])*s+t[2])*s+t[3])*s+t[4])*s+t[5])*l/(((((a[0]*s+a[1])*s+a[2])*s+a[3])*s+a[4])*s+1)}function De(e){const t=[...e.diagnosticos.residuosEstandarizados].sort((l,s)=>l-s),a=t.map((l,s)=>Fe((s+.5)/t.length)),r=A([...t,...a]),c=U({ancho:640,alto:360,margen:{izquierdo:66,derecho:28,superior:24,inferior:60},rangoX:r,rangoY:r,etiquetaX:"Cuantil normal teórico",etiquetaY:"Residuo estandarizado ordenado"}),n=t.map((l,s)=>`
        <circle
            cx="${c.x(a[s])}"
            cy="${c.y(l)}"
            r="4.5"
    ...
    `).join("");return N("Gráfico Q–Q de residuos","La proximidad de los puntos a la diagonal ofrece una evaluación visual de la normalidad residual.",P(`
            ${c.contenido}
            <line
                x1="${c.x(r.minimo)}"
                y1="${c.y(r.minimo)}"
            ...
            />
            ${n}
        `,"Gráfico cuantil cuantil de los residuos"))}function Ve(e){const t=e.diagnosticos.distanciaCook,a=4/e.n,r=Math.max(...t,a,1e-6),i=640,c=360,n={izquierdo:60,derecho:24,superior:24,inferior:58},s=(i-n.izquierdo-n.derecho)/t.length,o=ie(0,r*1.1,c-n.inferior,n.superior),d=t.map((p,m)=>{const v=n.izquierdo+m*s,h=c-n.inferior-o(p);return`
            <rect
                x="${v+Math.max(s*.15,1)}"
                y="${o(p)}"
        ...
        `}).join("");return N("Distancia de Cook",`El umbral orientativo mostrado es 4/n = ${E(a,4)}. Las barras rojas superan ese criterio y requieren revisión.`,P(`
        ...
        `,"Gráfico de distancia de Cook por observación"))}function Te(e,t="Y"){return`
        <section class="mt-10" data-seccion-graficos-regresion="true">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-2 gap-7">
                ${[Ne(e,t),Pe(e,t),Ie(e),Se(e),De(e),Ve(e)].filter(Boolean).join("")}
            </div>
        </section>
    `}function tt(){const e=document.createElement("section");let t=null,a=null;e.className=`
...
64, 3"
        ...
        <section
            ...
            aria-live="polite"
        ></section>
    `;const r=e.querySelector("#formulario-regresion"),i=e.querySelector("#mensaje-error-regresion"),c=e.querySelector("#resultados-regresion"),n=e.querySelector("[data-nombre-archivo]");return r.elements.tipoModelo.addEventListener("change",()=>{ae(r),c.classList.add("hidden"),F(i)}),r.elements.archivoDatos.addEventListener("change",async()=>{const l=r.elements.archivoDatos.files?.[0];if(l)try{r.elements.datos.value=await l.text(),n.textContent=`Archivo cargado: ${l.name}`,c.classList.add("hidden"),F(i)}catch{be(i,"No fue posible leer el archivo seleccionado.")}}),r.addEventListener("submit",l=>{l.preventDefault(),F(i);try{const s=Oe(r);t=ge(s.solicitud),a=s.metadatos,c.innerHTML=Ke(t,a),c.classList.remove("hidden"),c.scrollIntoView({behavior:"smooth",block:"start"})}catch(s){c.classList.add("hidden"),be(i,s instanceof Error?s.message:"No fue posible ajustar el modelo.")}}),e.addEventListener("submit",l=>{if(l.target.id!=="formulario-prediccion-regresion")return;l.preventDefault();const s=e.querySelector("#resultado-prediccion-regresion");try{const o=[...l.target.querySelectorAll("[data-predictor]")].map(p=>p.value.trim()===""?NaN:Number(p.value)),d=Ae(t,o);s.innerHTML=`
...
87; 8
89; 9
94; 10`}function f(e,t=4){return e==null?"—":e===1/0?"∞":e===-1/0?"−∞":Number.isFinite(e)?new Intl.NumberFormat("es-DO",{minimumFractionDigits:0,maximumFractionDigits:t}).format(e):"—"}function H(e){return e==null?"—":e<.001?"< 0.001":f(e,4)}function M(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function Xe(e,t){const a=[];return e.coeficientes.forEach((r,i)=>{const c=r.estimacion,n=f(Math.abs(c),6);if(i===0&&e.incluirIntercepto){a.push(f(c,6));return}const l=c>=0&&a.length?"+":"−",s=M(r.termino);a.length?a.push(`${l} ${n}·${s}`):a.push(`${c<0?"−":""}${n}·${s}`)}),`${M(t)}̂ = ${a.join(" ")}`}function L(e,t,a=""){return`
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    ${e.coeficientes.map(t=>`
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right font-black ${t.p<(e.alfa??.05)?"text-emerald-700":"text-slate-700"}">${H(t.p)}</td>
    ...
    `}function We(e){return e.diagnosticos.vif.length?`
        <div class="overflow-x-auto rounded-2xl border border-slate-200">
            <table class="min-w-full text-sm">
                <caption class="sr-only">
    ...
    `}function Ke(e,t){const a=e.anova.p!==null&&e.anova.p<(e.alfa??.05),r=f(e.ajuste.r2*100,2);return`
        ...
        <section class="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-7">
            ${L("R²",f(e.ajuste.r2,4),"Proporción explicada por el modelo")}
            ${L("R² ajustado",f(e.ajuste.r2Ajustado,4),"Ajustado por cantidad de predictores")}
            ${L("RMSE",f(e.ajuste.rmse,4),"Error cuadrático medio en unidades de Y")}
            ${L("MAE",f(e.ajuste.mae,4),"Error absoluto medio")}
        </section>
        ...
        <section class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-5">
            ${L("Durbin–Watson",f(e.diagnosticos.durbinWatson,4),"Valores cercanos a 2 son compatibles con ausencia de autocorrelación de primer orden")}
            ${L("Observaciones influyentes",f(e.diagnosticos.observacionesInfluyentes.length,0),"Criterios combinados de Cook, leverage y residuo estandarizado")}
            ${L("Error estándar",f(e.ajuste.errorEstandarEstimacion,4),"Desviación típica estimada de los residuos")}
        </section>

        ${Te(e,t.nombreDependiente)}
    ...
    `}function Ze(e,t){const a=["REGRESIÓN LINEAL Y MÚLTIPLE — LABORATORIO KERNEL","",`Variable dependiente: ${t.nombreDependiente}`,`Predictores: ${e.nombresPredictores.join(", ")}`,`n = ${e.n}`,`R² = ${f(e.ajuste.r2,4)}`,`R² ajustado = ${f(e.ajuste.r2Ajustado,4)}`,`F = ${f(e.anova.f,4)}`,`p del modelo = ${H(e.anova.p)}`,`RMSE = ${f(e.ajuste.rmse,4)}`,`Durbin–Watson = ${f(e.diagnosticos.durbinWatson,4)}`,"","Coeficientes:"];return e.coeficientes.forEach(r=>{a.push(`${r.termino}: b = ${f(r.estimacion,6)}, t = ${f(r.t,4)}, p ${H(r.p)}`)}),a.push("","Interpretación:"),e.interpretacion.forEach(r=>a.push(`- ${r}`)),a.push("","Diseño y desarrollo de las calculadoras: Miguel Antonio Leonardo Sepúlveda y Natanael Ureña Castillo."),a.join(`
`)}function Je(e){return`"${String(e??"").replaceAll('"','""')}"`}function et(e,t){const r=[["REGRESIÓN LINEAL Y MÚLTIPLE"],["Variable dependiente",t.nombreDependiente],["Predictores",e.nombresPredictores.join(", ")],["n",e.n],["R2",e.ajuste.r2],["R2 ajustado",e.ajuste.r2Ajustado],["RMSE",e.ajuste.rmse],["MAE",e.ajuste.mae],["F",e.anova.f],["p del modelo",e.anova.p],["Durbin-Watson",e.diagnosticos.durbinWatson],[],["Término","Coeficiente","Error estándar","t","p","IC inferior","IC superior"],...e.coeficientes.map(n=>[n.termino,n.estimacion,n.errorEstandar,n.t,n.p,n.intervaloConfianza.inferior,n.intervaloConfianza.superior]),[],["Observación","Observado","Predicho","Residuo","Residuo estandarizado","Leverage","Distancia de Cook"],...e.datosGrafico.map(n=>[n.observacion,n.observado,n.predicho,n.residuo,n.residuoEstandarizado,n.leverage,n.distanciaCook]),[],["Diseño y desarrollo","Miguel Antonio Leonardo Sepúlveda y Natanael Ureña Castillo"]].map(n=>n.map(Je).join(";")).join(`\r
`),i=new Blob([`\uFEFF${r}`],{type:"text/csv;charset=utf-8"}),c=document.createElement("a");c.href=URL.createObjectURL(i),c.download="regresion-lineal-multiple-kernel.csv",document.body.appendChild(c),c.click(),c.remove(),URL.revokeObjectURL(c.href)}function be(e,t){e.textContent=t,e.classList.remove("hidden"),e.scrollIntoView({behavior:"smooth",block:"center"})}function F(e){e.textContent="",e.classList.add("hidden")}export{tt as CalculadoraRegresionCompleta};
