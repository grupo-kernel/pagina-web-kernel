import{a as C}from"./estadisticaDosMuestrasRelacionadas-C0D8qKdR.js";import"./jstat-DiDocX-r.js";function P(){const e=document.createElement("section");e.className=`
        ...
        <section class="grid grid-cols-1 xl:grid-cols-[0.9fr_1.1fr] gap-7 mt-8">
            ...
            >
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-6">
                    ${h("medicion1","Medición 1","contador-medicion-1",`18
20
17.5
19
21`)}
                    ${h("medicion2","Medición 2","contador-medicion-2",`16
18.5
16
17
19`)}
        ...
        <section
            ...
            aria-live="polite"
        ></section>
    `;const a=e.querySelector("#formulario-muestras-relacionadas"),n=e.querySelector("#mensaje-error-relacionadas"),o=e.querySelector("#resultados-muestras-relacionadas"),r=a.elements.medicion1,s=a.elements.medicion2,l=e.querySelector("#contador-medicion-1"),m=e.querySelector("#contador-medicion-2"),u=sessionStorage.getItem("kernel-prueba-dos-relacionadas");["student-pareada","wilcoxon"].includes(u)&&(a.elements.prueba.value=u),sessionStorage.removeItem("kernel-prueba-dos-relacionadas");const p=()=>{l.textContent=k(w(r.value)),m.textContent=k(w(s.value))};return r.addEventListener("input",p),s.addEventListener("input",p),a.addEventListener("submit",x=>{x.preventDefault(),v(n);try{const d=a.elements.prueba.value;if(!d)throw new Error("Seleccione la prueba estadística que desea ejecutar.");const c=$(r.value,"La medición 1"),E=$(s.value,"La medición 2"),g=Number(a.elements.nivelConfianza.value),M=C({medicion1:c,medicion2:E,prueba:d,nivelConfianza:g});o.innerHTML=z(M,g),o.classList.remove("hidden"),o.scrollIntoView({behavior:"smooth",block:"start"})}catch(d){o.classList.add("hidden"),j(n,d instanceof Error?d.message:"No fue posible ejecutar el análisis.")}}),e.addEventListener("click",x=>{const d=x.target.closest("[data-action]");if(!d)return;const c=d.dataset.action;if(c==="volver-laboratorio"){window.location.hash="/laboratorioKernel";return}if(c==="cargar-ejemplo"){a.elements.prueba.value||(a.elements.prueba.value="student-pareada"),r.value=`18
20
17.5
19
21
...
15
20
17`,p(),v(n),o.classList.add("hidden");return}c==="limpiar"&&(a.reset(),r.value="",s.value="",p(),v(n),o.innerHTML="",o.classList.add("hidden"))}),e}function f(e,a){return`
    ...
    `}function $(e,a){const n=e.trim();if(!n)throw new Error(`${a} no contiene datos.`);const o=n.split(/[\s;]+/).filter(Boolean),r=o.map(l=>Number(l.replace(",","."))),s=r.findIndex(l=>!Number.isFinite(l));if(s!==-1)throw new Error(`${a} contiene un valor no válido: "${o[s]}".`);if(r.length<2)throw new Error(`${a} debe contener al menos dos observaciones.`);return r}function w(e){return e.trim()?e.trim().split(/[\s;]+/).filter(Boolean).length:0}function k(e){return e===1?"1 valor":`${e} valores`}function j(e,a){e.textContent=a,e.classList.remove("hidden")}function v(e){e.textContent="",e.classList.add("hidden")}function i(e,a=4){return Number.isFinite(e)?new Intl.NumberFormat("es-DO",{minimumFractionDigits:0,maximumFractionDigits:a}).format(e):"—"}function L(e){return Number.isFinite(e)?e<.001?"< 0.001":i(e,4):"—"}function z(e,a=.95){const n=1-a,o=e.valorP<n,r=n.toFixed(2),s=Math.round(a*100),l=e.id==="t-student-relacionadas"?e.diferenciaMedia:e.diferencias.mediana,m=D(l),u=o?`Se observa evidencia estadísticamente significativa de una diferencia entre las dos mediciones al nivel α = ${r}, correspondiente a un nivel de confianza del ${s} %. ${m}`:`No se observa evidencia estadísticamente significativa de una diferencia entre las dos mediciones al nivel α = ${r}, correspondiente a un nivel de confianza del ${s} %. ${m}`;return`
    ...
    `}function D(e){return Number.isFinite(e)?e>0?"La dirección estimada favorece valores mayores en la medición 1.":e<0?"La dirección estimada favorece valores mayores en la medición 2.":"La diferencia central estimada es igual a cero.":""}function b(e,a,n){return`
    ...
    `}function N(e){if(e.id==="wilcoxon-relacionadas")return`
    ...
    `}export{P as CalculadoraDosMuestrasRelacionadas};
