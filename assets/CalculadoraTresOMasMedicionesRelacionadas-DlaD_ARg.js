import{j as ee,a as ae}from"./jstat-DiDocX-r.js";import{p as te,b as oe}from"./estadisticaDosMuestrasRelacionadas-C0D8qKdR.js";const q=ee.jStat??ae?.jStat;if(!q)throw new Error("No fue posible cargar el motor estadístico jStat.");const Z=e=>Math.min(1,Math.max(0,e));function K(e){if(!Number.isFinite(e)||e<=0||e>=1)throw new RangeError("El nivel de confianza debe estar entre 0 y 1.")}function re(e){return typeof e=="string"?e.trim()===""?Number.NaN:Number(e.trim().replace(",",".")):Number(e)}function A(e){return e.reduce((a,t)=>a+t,0)/e.length}function se(e){const a=[...e].sort((o,r)=>o-r),t=Math.floor(a.length/2);return a.length%2?a[t]:(a[t-1]+a[t])/2}function ne(e,a=A(e)){return e.reduce((t,o)=>t+(o-a)**2,0)/(e.length-1)}function ie(e){const a=A(e),t=ne(e,a);return{n:e.length,media:a,mediana:se(e),varianza:t,desviacionEstandar:Math.sqrt(t),minimo:Math.min(...e),maximo:Math.max(...e)}}function ce(e,a){const t=Array.isArray(e),o=t?e:e?.valores;if(!Array.isArray(o))throw new TypeError(`La medición ${a+1} debe contener un arreglo llamado valores.`);const r=o.map(re),n=r.findIndex(i=>!Number.isFinite(i));if(n!==-1)throw new TypeError(`La medición ${a+1} contiene un valor no numérico en la posición ${n+1}.`);if(r.length<2)throw new RangeError(`La medición ${a+1} debe contener al menos dos observaciones.`);return{nombre:(t?"":String(e?.nombre??"").trim())||`Medición ${a+1}`,valores:r,...ie(r)}}function Q(e){if(!Array.isArray(e))throw new TypeError("Las mediciones deben proporcionarse mediante un arreglo.");if(e.length<3)throw new RangeError("El análisis requiere al menos tres mediciones relacionadas.");const a=e.map(ce),t=a[0].n;if(a.find(({n})=>n!==t))throw new RangeError("Todas las mediciones deben contener el mismo número de observaciones y conservar el mismo orden de participantes.");const r=a.map(({nombre:n})=>n);if(new Set(r).size!==r.length)throw new Error("Cada medición debe tener un nombre diferente.");return a}function J(e){const a=e[0].n;return Array.from({length:a},(t,o)=>e.map(({valores:r})=>r[o]))}function N(e,a,t){return e===1/0?0:Z(1-q.centralF.cdf(e,a,t))}function le(e,a){return Z(1-q.chisquare.cdf(e,a))}function U(e){const a=e.map((o,r)=>({...o,indiceOriginal:r})).sort((o,r)=>o.valorP-r.valorP);let t=0;return a.forEach((o,r)=>{const n=Math.min(1,o.valorP*(a.length-r));o.valorPAjustado=Math.max(t,n),t=o.valorPAjustado}),a.sort((o,r)=>o.indiceOriginal-r.indiceOriginal).map(({indiceOriginal:o,...r})=>r)}function X(e,a){const t=[];for(let o=0;o<e.length-1;o+=1)for(let r=o+1;r<e.length;r+=1)t.push(a(e[o],e[r],o,r));return t}function z(e,a){const t=e.length,o=a[0].length,r=a.length;return Array.from({length:t},(n,s)=>Array.from({length:o},(i,l)=>{let c=0;for(let d=0;d<r;d+=1)c+=e[s][d]*a[d][l];return c}))}function _(e){return e.reduce((a,t,o)=>a+t[o],0)}function de(e){const a=e.length,t=e[0].length,o=Array.from({length:t},(r,n)=>A(e.map(s=>s[n])));return Array.from({length:t},(r,n)=>Array.from({length:t},(s,i)=>{let l=0;for(let c=0;c<a;c+=1)l+=(e[c][n]-o[n])*(e[c][i]-o[i]);return l/(a-1)}))}function me(e){const a=e.length,t=e[0].length,o=de(e),r=Array.from({length:t},(f,w)=>Array.from({length:t},(x,$)=>(w===$?1:0)-1/t)),n=z(z(r,o),r),s=_(n),i=z(n,n),l=_(i),c=1/(t-1);let d=l===0?1:s**2/((t-1)*l);d=Math.min(1,Math.max(c,d));const u=a*(t-1)*d-2,b=(t-1)*(a-1-(t-1)*d);let p=b<=0?1:u/b;return p=Math.min(1,Math.max(c,p)),{limiteInferior:c,greenhouseGeisser:d,huynhFeldt:p,covarianzas:o}}function ue(e){const a=Math.abs(e);return a<.01?"Prácticamente nulo":a<.06?"Pequeño":a<.14?"Moderado":"Grande"}function pe(e){const a=Math.abs(e);return a<.1?"Muy pequeño":a<.3?"Pequeño":a<.5?"Moderado":"Grande"}function fe(e,a){const t=1-a,o=X(e,(r,n)=>{const s=te(r.valores,n.valores,{nivelConfianza:a});return{medicion1:r.nombre,medicion2:n.nombre,diferenciaMedia:s.diferenciaMedia,estadisticoT:s.estadistico.valor,gradosLibertad:s.gradosLibertad,valorP:s.valorP,intervaloConfianza:s.intervaloConfianza,dZ:s.tamanioEfecto.dZ}});return{id:"t-pareadas-holm",nombre:"Comparaciones t pareadas con ajuste de Holm",metodo:"Comparaciones pareadas bilaterales con ajuste secuencial de Holm sobre los valores p.",recomendado:!1,comparaciones:U(o).map(r=>({...r,significativo:r.valorPAjustado<t}))}}function be(e,a){const t=1-a,o=X(e,(r,n)=>{const s=oe(r.valores,n.valores);return{medicion1:r.nombre,medicion2:n.nombre,estadisticoW:s.estadistico.valor,estadisticoZ:s.estadisticoZ,valorP:s.valorP,correlacionBiserialRangos:s.tamanioEfecto.correlacionBiserialRangos,r:s.tamanioEfecto.r,nEfectivo:s.nEfectivo,cantidadCeros:s.cantidadCeros}});return{id:"wilcoxon-holm",nombre:"Comparaciones de Wilcoxon con ajuste de Holm",metodo:"Comparaciones pareadas de rangos con signo y ajuste secuencial de Holm sobre los valores p.",recomendado:!1,comparaciones:U(o).map(r=>({...r,significativo:r.valorPAjustado<t}))}}function xe(e){const a=e.map((s,i)=>({valor:s,indice:i,rango:0})).sort((s,i)=>s.valor-i.valor);let t=0,o=0,r=0;for(;t<a.length;){let s=t+1;for(;s<a.length&&a[s].valor===a[t].valor;)s+=1;const i=s-t,l=(t+1+s)/2;for(let c=t;c<s;c+=1)a[c].rango=l;i>1&&(r+=1,o+=i**3-i),t=s}const n=new Array(e.length).fill(0);return a.forEach(({indice:s,rango:i})=>{n[s]=i}),{rangos:n,correccionEmpates:o,cantidadGruposEmpatados:r}}function he(e,a={}){const{nivelConfianza:t=.95}=a;K(t);const o=Q(e),r=J(o),n=r.length,s=o.length,i=r.flat(),l=A(i),c=r.map(M=>A(M)),d=i.reduce((M,C)=>M+(C-l)**2,0),u=n*o.reduce((M,C)=>M+(C.media-l)**2,0),b=s*c.reduce((M,C)=>M+(C-l)**2,0),p=Math.max(0,d-u-b),f=s-1,w=n-1,x=f*w,$=u/f,j=p/x,g=j===0?$===0?0:1/0:$/j,h=N(g,f,x),P=u+p===0?0:u/(u+p),F=u+p+b===0?0:u/(u+p+b),E=me(r),R=f*E.greenhouseGeisser,H=x*E.greenhouseGeisser,I=f*E.huynhFeldt,V=x*E.huynhFeldt,G=N(g,R,H),O=N(g,I,V),k=1-t,D=fe(o,t);return D.recomendado=G<k,{id:"anova-medidas-repetidas",nombre:"ANOVA de medidas repetidas",metodo:"ANOVA unifactorial intra-sujetos para tres o más mediciones completas y relacionadas.",mediciones:o,nSujetos:n,numeroMediciones:s,mediaGeneral:l,mediasSujetos:c,estadistico:{simbolo:"F",valor:g},gradosLibertad:{numerador:f,denominador:x},valorP:h,nivelConfianza:t,alfa:k,significativo:h<k,tablaAnova:{mediciones:{sumaCuadrados:u,gradosLibertad:f,cuadradoMedio:$},sujetos:{sumaCuadrados:b,gradosLibertad:w},error:{sumaCuadrados:p,gradosLibertad:x,cuadradoMedio:j},total:{sumaCuadrados:d,gradosLibertad:n*s-1}},esfericidad:{...E,greenhouseGeisser:{epsilon:E.greenhouseGeisser,gradosLibertadNumerador:R,gradosLibertadDenominador:H,valorP:G,significativo:G<k},huynhFeldt:{epsilon:E.huynhFeldt,gradosLibertadNumerador:I,gradosLibertadDenominador:V,valorP:O,significativo:O<k},nota:"Se informan correcciones de Greenhouse–Geisser y Huynh–Feldt. La calculadora no sustituye una prueba formal de esfericidad ni el examen de la matriz de covarianzas."},tamanioEfecto:{etaCuadradoParcial:P,etaCuadradoGeneralizado:F,interpretacion:ue(P)},postHoc:D}}function ge(e,a={}){const{nivelConfianza:t=.95}=a;K(t);const o=Q(e),r=J(o),n=r.length,s=o.length,i=new Array(s).fill(0);let l=0,c=0;r.forEach(g=>{const h=xe(g);h.rangos.forEach((P,F)=>{i[F]+=P}),l+=h.correccionEmpates,c+=h.cantidadGruposEmpatados});const d=12/(n*s*(s+1))*i.reduce((g,h)=>g+h**2,0)-3*n*(s+1),u=1-l/(n*(s**3-s)),b=u<=0?0:d/u,p=s-1,f=le(b,p),w=n*(s-1)===0?0:b/(n*(s-1)),x=1-t,$=o.map((g,h)=>({...g,sumaRangos:i[h],rangoMedio:i[h]/n})),j=be(o,t);return j.recomendado=f<x,{id:"friedman",nombre:"Prueba de Friedman",metodo:"Comparación no paramétrica de tres o más mediciones relacionadas mediante rangos dentro de cada participante.",mediciones:$,nSujetos:n,numeroMediciones:s,estadistico:{simbolo:"χ²_F",valor:b,sinCorreccion:d},gradosLibertad:{valor:p},valorP:f,nivelConfianza:t,alfa:x,significativo:f<x,empates:{factorCorreccion:u,sumaCorreccion:l,cantidadGruposEmpatados:c},tamanioEfecto:{kendallW:w,interpretacion:pe(w)},postHoc:j}}function ve({mediciones:e,prueba:a,nivelConfianza:t=.95}){switch(a){case"anova-medidas-repetidas":case"anova-repetidas":return he(e,{nivelConfianza:t});case"friedman":return ge(e,{nivelConfianza:t});default:throw new Error("La prueba solicitada no está disponible.")}}function Le(){const e=document.createElement("section");e.className=`
...
19"
    ...
    `}function ye(e,a){const t=e.trim();if(!t)throw new Error(`${a} no contiene datos.`);const o=t.split(/[\s;]+/).filter(Boolean),r=o.map(s=>Number(s.replace(",","."))),n=r.findIndex(s=>!Number.isFinite(s));if(n!==-1)throw new Error(`${a} contiene un valor no válido: "${o[n]}".`);if(r.length<2)throw new Error(`${a} debe contener al menos dos observaciones.`);return r}function we(e){const a=[...e.querySelectorAll("[data-medicion]")];if(a.length<3)throw new Error("El análisis requiere al menos tres mediciones.");return a.map((t,o)=>{const r=t.querySelector('[data-campo="nombre"]'),n=t.querySelector('[data-campo="valores"]'),s=r.value.trim()||`Medición ${o+1}`;return{nombre:s,valores:ye(n.value,s)}})}function $e(e,a){e.elements.prueba.value=e.elements.prueba.value||"anova-medidas-repetidas",a.innerHTML=`
        ${v(1)}
        ${v(2)}
        ${v(3)}
...
20`];[...a.querySelectorAll("[data-medicion]")].forEach((n,s)=>{n.querySelector('[data-campo="nombre"]').value=t[s],n.querySelector('[data-campo="valores"]').value=o[s],Y(n)})}function Y(e){const a=e.querySelector('[data-campo="valores"]'),t=e.querySelector("[data-contador]"),o=a.value.trim()?a.value.trim().split(/[\s;]+/).filter(Boolean).length:0;t.textContent=o===1?"1 valor":`${o} valores`}document.addEventListener("input",e=>{const a=e.target.closest('#formulario-mediciones-relacionadas [data-campo="valores"]');if(!a)return;const t=a.closest("[data-medicion]");t&&Y(t)});function B(e,a){e.textContent=a,e.classList.remove("hidden")}function T(e){e.textContent="",e.classList.add("hidden")}function y(e,a=4){return Number.isFinite(e)?new Intl.NumberFormat("es-DO",{minimumFractionDigits:0,maximumFractionDigits:a}).format(e):e===1/0?"∞":"—"}function S(e){return Number.isFinite(e)?e<.001?"< 0.001":y(e,4):"—"}function Ee(e){const a=e.id==="anova-medidas-repetidas",t=a?e.esfericidad.greenhouseGeisser.valorP:e.valorP,o=t<e.alfa,r=Math.round(e.nivelConfianza*100),n=a?"Greenhouse–Geisser":"sin corrección adicional",s=o?`Se observa evidencia estadísticamente significativa de diferencias entre las mediciones al nivel α = ${e.alfa.toFixed(2)}. Para la decisión principal se utilizó ${n}.`:`No se observa evidencia estadísticamente significativa de diferencias entre las mediciones al nivel α = ${e.alfa.toFixed(2)}. Para la decisión principal se utilizó ${n}.`;return`
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 mt-6">
            <article class="rounded-2xl border border-blue-200 bg-blue-50 p-6">
                ...
                <dl class="grid grid-cols-2 gap-4">
                    ${m("SC mediciones",a.mediciones.sumaCuadrados)}
                    ${m("gl mediciones",a.mediciones.gradosLibertad)}
                    ${m("CM mediciones",a.mediciones.cuadradoMedio)}
                    ${m("SC sujetos",a.sujetos.sumaCuadrados)}
                    ${m("SC error",a.error.sumaCuadrados)}
                    ${m("gl error",a.error.gradosLibertad)}
                    ${m("CM error",a.error.cuadradoMedio)}
                    ${m("SC total",a.total.sumaCuadrados)}
                </dl>
            </article>
            ...
            <article class="rounded-2xl border border-violet-200 bg-violet-50 p-6">
                ...
                <dl class="grid grid-cols-2 gap-4">
                    ${m("ε Greenhouse–Geisser",t.epsilon)}
                    ${m("p Greenhouse–Geisser",t.valorP,!0)}
                    ${m("ε Huynh–Feldt",o.epsilon)}
                    ${m("p Huynh–Feldt",o.valorP,!0)}
                    ${m("Eta cuadrado parcial",e.tamanioEfecto.etaCuadradoParcial)}
                    ${m("Eta cuadrado generalizado",e.tamanioEfecto.etaCuadradoGeneralizado)}
                </dl>
    ...
    `}function ke(e){return`
        <article class="mt-6 rounded-2xl border border-violet-200 bg-violet-50 p-6">
            ...
            <dl class="grid grid-cols-2 md:grid-cols-3 gap-4">
                ${m("Grados de libertad",e.gradosLibertad.valor)}
                ${m("Factor de corrección por empates",e.empates.factorCorreccion)}
                ${m("Grupos de empates",e.empates.cantidadGruposEmpatados)}
                ${m("W de Kendall",e.tamanioEfecto.kendallW)}
                ${m("Magnitud",e.tamanioEfecto.interpretacion,!1,!0)}
            </dl>
        </article>
    `}function m(e,a,t=!1,o=!1){const r=o?a:t?S(a):y(a);return`
        <div>
    ...
    `}function Ce(e){return e?.comparaciones?.length?`
    ...
    `:""}export{Le as CalculadoraTresOMasMedicionesRelacionadas};
