function p(){const e=document.createElement("section");return e.className=`
    ...
    `,e.innerHTML=`
        ...
        <section class="mt-10">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-7">
                ...
                ${o({etiqueta:"Resultado de conteo",titulo:"Poisson y binomial negativa",descripcion:"Modele el número esperado de eventos, compare la dispersión y estime razones de tasas de incidencia con exposición opcional.",caracteristicas:["Selección automática o manual","IRR e intervalos de confianza","Dispersión, AIC, BIC y deviance","Exceso de ceros e influencia","Predicción de tasas y conteos"],ruta:"calculadoraRegresionConteo",clase:"amber"})}
            </div>
        </section>

        <section class="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-5">
            ${r("Defina el desenlace","Especifique la variable dependiente, su escala y qué representa un valor alto, bajo, un evento codificado como 1 o un conteo acumulado.")}
            ${r("Separe explicación y predicción","Un modelo explicativo prioriza interpretación y control de confusión; uno predictivo exige validación, evaluación fuera de muestra y atención al rendimiento.")}
            ${r("Revise los supuestos","La significación estadística no compensa una forma funcional incorrecta, colinealidad, observaciones influyentes, sobredispersión, mala calibración o sesgo del diseño.")}
        </section>
    ...
    `,e.addEventListener("click",t=>{const i=t.target.closest("[data-route]");if(t.target.closest("[data-action='volver-laboratorio']")){window.location.hash="/laboratorioKernel";return}i?.dataset.route&&(window.location.hash=`/${i.dataset.route}`)}),e}function o({etiqueta:e,titulo:t,descripcion:i,caracteristicas:s,ruta:l,clase:n}){const c={rose:{borde:"border-rose-200",fondo:"bg-rose-50",etiqueta:"text-rose-700",boton:"bg-rose-700 hover:bg-rose-800",icono:"bg-rose-100 text-rose-700 border-rose-200"},amber:{borde:"border-amber-200",fondo:"bg-amber-50",etiqueta:"text-amber-700",boton:"bg-amber-600 hover:bg-amber-700",icono:"bg-amber-100 text-amber-700 border-amber-200"},indigo:{borde:"border-indigo-200",fondo:"bg-indigo-50",etiqueta:"text-indigo-700",boton:"bg-indigo-700 hover:bg-indigo-800",icono:"bg-indigo-100 text-indigo-700 border-indigo-200"}},a=c[n]||c.indigo;return`
        <article class="h-full rounded-3xl border ${a.borde} bg-white p-6 md:p-8 shadow-xl flex flex-col">
            <div class="w-16 h-16 rounded-2xl border ${a.icono} flex items-center justify-center mb-6">
                ${u(n)}
            </div>

            <p class="uppercase tracking-widest text-xs font-black ${a.etiqueta} mb-2">
                ${e}
            </p>
            ...
            <ul class="space-y-3 mb-8">
                ${s.map(d=>`
                    <li class="flex gap-3 text-slate-700">
                        <span class="font-black ${a.etiqueta}">✓</span>
                        <span>${d}</span>
                    </li>
                `).join("")}
            </ul>

            <div class="mt-auto rounded-2xl ${a.fondo} border ${a.borde} p-4 text-sm text-slate-700">
            ...
            <button
                type="button"
                data-route="${l}"
                class="mt-6 inline-flex items-center justify-center rounded-xl ${a.boton} px-7 py-4 text-white font-black transition-colors shadow-lg"
            >
                Abrir calculadora
