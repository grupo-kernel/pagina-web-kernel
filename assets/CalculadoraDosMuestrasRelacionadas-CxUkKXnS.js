...
17`,u(),v(n),o.classList.add("hidden");return}d==="limpiar"&&(a.reset(),r.value="",s.value="",u(),v(n),o.innerHTML="",o.classList.add("hidden"))}),e}function f(e,a){return`
    ...
    `}function $(e,a){const n=e.trim();if(!n)throw new Error(`${a} no contiene datos.`);const o=n.split(/[\s;]+/).filter(Boolean),r=o.map(l=>Number(l.replace(",","."))),s=r.findIndex(l=>!Number.isFinite(l));if(s!==-1)throw new Error(`${a} contiene un valor no válido: "${o[s]}".`);if(r.length<2)throw new Error(`${a} debe contener al menos dos observaciones.`);return r}function w(e){return e.trim()?e.trim().split(/[\s;]+/).filter(Boolean).length:0}function k(e){return e===1?"1 valor":`${e} valores`}function j(e,a){e.textContent=a,e.classList.remove("hidden")}function v(e){e.textContent="",e.classList.add("hidden")}function i(e,a=4){return Number.isFinite(e)?new Intl.NumberFormat("es-DO",{minimumFractionDigits:0,maximumFractionDigits:a}).format(e):"—"}function L(e){return Number.isFinite(e)?e<.001?"< 0.001":i(e,4):"—"}function z(e,a=.95){const n=1-a,o=e.valorP<n,r=n.toFixed(2),s=Math.round(a*100),l=e.id==="t-student-relacionadas"?e.diferenciaMedia:e.diferencias.mediana,m=D(l),p=o?`Se observa evidencia estadísticamente significativa de una diferencia entre las dos mediciones al nivel α = ${r}, correspondiente a un nivel de confianza del ${s} %. ${m}`:`No se observa evidencia estadísticamente significativa de una diferencia entre las dos mediciones al nivel α = ${r}, correspondiente a un nivel de confianza del ${s} %. ${m}`;return`
