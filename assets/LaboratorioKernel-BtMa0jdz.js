import{c as x}from"./authService-CgfoSPdf.js";import{M as d,i as a}from"./index-qgiCDO7O.js";import"./firebase-BzP43VG7.js";const l=e=>Object.freeze(e),u=Object.freeze([l({numero:"01",id:"orientar",titulo:"Orientar",descripcion:"Defina el objetivo, el tipo de variable y el diseño antes de elegir una prueba.",icono:"bx-check-shield",estado:"Disponible",detalleEstado:"Asistente KSDE 2.0",accion:l({tipo:"ruta",valor:"asistentePruebas",etiqueta:"Iniciar orientación"}),fondoIcono:"bg-blue-100",colorIcono:"text-blue-700",bordeIcono:"border-blue-200"}),l({numero:"02",id:"analizar",titulo:"Analizar",descripcion:"Seleccione una herramienta, introduzca sus datos y ejecute el procedimiento estadístico.",icono:"bx-calculator",estado:"Disponible",detalleEstado:"Catálogo de análisis",accion:l({tipo:"ancla",valor:"catalogo-analisis",etiqueta:"Ver herramientas"}),fondoIcono:"bg-emerald-100",colorIcono:"text-emerald-700",bordeIcono:"border-emerald-200"}),l({numero:"03",id:"interpretar",titulo:"Interpretar",descripcion:"Examine el estadístico, el valor p, el intervalo de confianza y el tamaño del efecto.",icono:"bx-line-chart",estado:"Integrado",detalleEstado:"Dentro de las calculadoras",accion:null,fondoIcono:"bg-amber-100",colorIcono:"text-amber-700",bordeIcono:"border-amber-200"}),l({numero:"04",id:"reportar",titulo:"Reportar",descripcion:"Utilice las orientaciones de cada herramienta para comunicar resultados con rigor y contexto.",icono:"bx-edit-alt",estado:"Integrado",detalleEstado:"Según la herramienta",accion:null,fondoIcono:"bg-violet-100",colorIcono:"text-violet-700",bordeIcono:"border-violet-200"})]),b=Object.freeze([l({id:"preparar-describir",titulo:"Preparar y describir",descripcion:"Conozca la estructura de sus datos y planifique el tamaño necesario antes de la inferencia.",icono:"bx-bar-chart-alt-2",modulos:l(["estadistica-descriptiva","tamano-muestra"])}),l({id:"comparar-relacionar",titulo:"Comparar y relacionar",descripcion:"Contraste grupos, mediciones y relaciones entre variables cuantitativas o categóricas.",icono:"bx-group",modulos:l(["comparacion-grupos","correlacion-asociacion"])}),l({id:"modelar-evaluar",titulo:"Modelar y evaluar",descripcion:"Construya modelos y estudie la calidad de instrumentos, ítems y evaluaciones.",icono:"bx-trending-up",modulos:l(["regresion","cuestionarios-fiabilidad","evaluacion-educativa"])}),l({id:"consultar-aprender",titulo:"Consultar y aprender",descripcion:"Revise fundamentos, supuestos, alternativas, errores frecuentes y formas de reporte.",icono:"bx-library",modulos:l(["biblioteca-metodologica"])})]),p="asistente-pruebas",m=new Map(d.map(e=>[e.id,e]));function t(e){return String(e??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function f(e){return`
    ...
    `}function k(e){const r=e.modulos.map(s=>m.get(s)).filter(Boolean);return`
    ...
    `}function c(e){e&&(window.location.hash=`/${e}`)}function $(e,r){const s=document.getElementById(r);if(!s||!e.contains(s))return;const o=window.matchMedia?.("(prefers-reduced-motion: reduce)").matches;s.scrollIntoView({behavior:o?"auto":"smooth",block:"start"}),s.focus({preventScroll:!0})}function E(){const e=d.filter(o=>o.id!==p).length,r=document.createElement("section");r.className="w-full max-w-7xl mx-auto px-4 py-10 md:px-8 font-sans",r.innerHTML=`
    ...
    `,r.addEventListener("click",o=>{const i=o.target.closest("[data-route]");if(i&&r.contains(i)){o.preventDefault(),c(i.dataset.route);return}const n=o.target.closest("[data-scroll-target]");n&&r.contains(n)&&$(r,n.dataset.scrollTarget)});const s=r.querySelector("#cerrar-sesion-kernel");return s?.addEventListener("click",async()=>{s.disabled=!0,s.textContent="Cerrando…";try{await x(),c("home")}catch(o){console.error("[Kernel] No fue posible cerrar la sesión.",o),s.disabled=!1,s.textContent="Cerrar sesión"}}),r}export{E as LaboratorioKernel};
