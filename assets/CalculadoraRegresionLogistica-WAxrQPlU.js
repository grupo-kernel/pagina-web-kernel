function te(e,t,o=5){if(!Array.isArray(t)||t.length<o)throw new Error(`${e} debe contener al menos ${o} observaciones.`);const a=t.map(s=>s==null||typeof s=="string"&&s.trim()===""?NaN:Number(s));if(!a.every(Number.isFinite))throw new Error(`${e} contiene valores no numéricos o no finitos.`);return a}function fe(e){return Array.from({length:e},(t,o)=>Array.from({length:e},(a,s)=>o===s?1:0))}function he(e){return e[0].map((t,o)=>e.map(a=>a[o]))}function ge(e,t){return e.map(o=>t[0].map((a,s)=>o.reduce((l,i,c)=>l+i*t[c][s],0)))}function S(e,t){return e.map(o=>o.reduce((a,s,l)=>a+s*t[l],0))}function A(e,t){return e.reduce((o,a,s)=>o+a*t[s],0)}function H(e){const t=e.length,o=e.map((a,s)=>[...a.map(Number),...fe(t)[s]]);for(let a=0;a<t;a+=1){let s=a;for(let i=a+1;i<t;i+=1)Math.abs(o[i][a])>Math.abs(o[s][a])&&(s=i);if(Math.abs(o[s][a])<1e-12)throw new Error("La matriz de información es singular. Elimine predictores redundantes o perfectamente colineales.");[o[a],o[s]]=[o[s],o[a]];const l=o[a][a];o[a]=o[a].map(i=>i/l);for(let i=0;i<t;i+=1){if(i===a)continue;const c=o[i][a];o[i]=o[i].map((r,n)=>r-c*o[a][n])}}return o.map(a=>a.slice(t))}function se(e,t){const o=e[0].length;return Array.from({length:o},(a,s)=>[...t?[1]:[],...e.map(l=>l[s])])}function G(e){const t=Math.max(-35,Math.min(35,e));if(t>=0)return 1/(1+Math.exp(-t));const o=Math.exp(t);return o/(1+o)}function F(e,t){return e.reduce((o,a,s)=>{const l=Math.max(1e-12,Math.min(.999999999999,t[s]));return o+a*Math.log(l)+(1-a)*Math.log(1-l)},0)}function ve(e){const t=e<0?-1:1,o=Math.abs(e)/Math.sqrt(2),a=1/(1+.3275911*o),n=1-((((1.061405429*a+-1.453152027)*a+1.421413741)*a+-.284496736)*a+.254829592)*a*Math.exp(-o*o);return .5*(1+t*n)}function $e(e){return Math.max(0,Math.min(1,2*(1-ve(Math.abs(e)))))}function X(e){const t=[676.5203681218851,-1259.1392167224028,771.3234287776531,-176.6150291621406,12.507343278686905,-.13857109526572012,9984369578019572e-21,15056327351493116e-23];if(e<.5)return Math.log(Math.PI)-Math.log(Math.sin(Math.PI*e))-X(1-e);let o=.9999999999998099;const a=e-1;t.forEach((l,i)=>{o+=l/(a+i+1)});const s=a+t.length-.5;return .5*Math.log(2*Math.PI)+(a+.5)*Math.log(s)-s+Math.log(o)}function ye(e,t){if(t<=0)return 0;if(t<e+1){let c=1/e,r=c,n=e;for(let d=1;d<=300&&(n+=1,c*=t/n,r+=c,!(Math.abs(c)<Math.abs(r)*1e-14));d+=1);return r*Math.exp(-t+e*Math.log(t)-X(e))}let o=t+1-e,a=1/1e-300,s=1/o,l=s;for(let c=1;c<=300;c+=1){const r=-c*(c-e);o+=2,s=r*s+o,Math.abs(s)<1e-300&&(s=1e-300),a=o+r/a,Math.abs(a)<1e-300&&(a=1e-300),s=1/s;const n=s*a;if(l*=n,Math.abs(n-1)<1e-14)break}return 1-Math.exp(-t+e*Math.log(t)-X(e))*l}function ie(e,t){return!(e>=0)||!(t>0)?null:Math.max(0,Math.min(1,1-ye(t/2,e/2)))}function we(e){const t=[-39.6968302866538,220.946098424521,-275.928510446969,138.357751867269,-30.6647980661472,2.50662827745924],o=[-54.4760987982241,161.585836858041,-155.698979859887,66.8013118877197,-13.2806815528857],a=[-.00778489400243029,-.322396458041136,-2.40075827716184,-2.54973253934373,4.37466414146497,2.93816398269878],s=[.00778469570904146,.32246712907004,2.445134137143,3.75440866190742],l=.02425,i=1-l;if(e<=0||e>=1)throw new Error("La probabilidad debe estar entre 0 y 1.");if(e<l){const n=Math.sqrt(-2*Math.log(e));return(((((a[0]*n+a[1])*n+a[2])*n+a[3])*n+a[4])*n+a[5])/((((s[0]*n+s[1])*n+s[2])*n+s[3])*n+1)}if(e>i){const n=Math.sqrt(-2*Math.log(1-e));return-(((((a[0]*n+a[1])*n+a[2])*n+a[3])*n+a[4])*n+a[5])/((((s[0]*n+s[1])*n+s[2])*n+s[3])*n+1)}const c=e-.5,r=c*c;return(((((t[0]*r+t[1])*r+t[2])*r+t[3])*r+t[4])*r+t[5])*c/(((((o[0]*r+o[1])*r+o[2])*r+o[3])*r+o[4])*r+1)}function ae(e,t){const o=e[0].length,a=Array.from({length:o},()=>Array(o).fill(0));return e.forEach((s,l)=>{const i=Math.max(t[l]*(1-t[l]),1e-12);for(let c=0;c<o;c+=1)for(let r=0;r<o;r+=1)a[c][r]+=i*s[c]*s[r]}),a}function ke(e,t,o={}){const a=o.maxIteraciones??100,s=o.tolerancia??1e-8,l=e[0].length;let i=Array(l).fill(0),c=e.map(()=>.5),r=F(t,c),n=!1,d=0;for(let v=1;v<=a;v+=1){d=v,c=e.map(w=>G(A(w,i)));const x=ae(e,c),g=H(x),u=Array(l).fill(0);e.forEach((w,C)=>{const q=t[C]-c[C];w.forEach((L,P)=>{u[P]+=L*q})});const $=S(g,u);let y=1,E=i.map((w,C)=>w+$[C]),M=e.map(w=>G(A(w,E))),I=F(t,M);for(;I<r-1e-10&&y>1/1024;)y/=2,E=i.map((w,C)=>w+y*$[C]),M=e.map(w=>G(A(w,E))),I=F(t,M);const N=Math.max(...E.map((w,C)=>Math.abs(w-i[C]))),_=Math.abs(I-r);if(i=E,c=M,r=I,N<s&&_<s){n=!0;break}if(i.some(w=>Math.abs(w)>100))break}const p=ae(e,c),b=H(p);if(!i.every(Number.isFinite)||!c.every(Number.isFinite)||!b.flat().every(Number.isFinite)||!Number.isFinite(r))throw new Error("El ajuste produjo resultados no finitos. Revise valores extremos, escala de los predictores y separación.");return{beta:i,probabilidades:c,logLik:r,covarianza:b,informacion:p,convergio:n,iteraciones:d}}function Me(e,t){if(!t){const l=e.map(()=>.5);return{probabilidades:l,logLik:F(e,l)}}const o=e.reduce((l,i)=>l+i,0)/e.length,a=Math.max(1e-12,Math.min(1-1e-12,o)),s=e.map(()=>a);return{probabilidades:s,logLik:F(e,s)}}function Ce(e,t,o){let a=0,s=0,l=0,i=0;e.forEach((x,g)=>{const u=t[g]>=o?1:0;x===1&&u===1&&(a+=1),x===0&&u===0&&(s+=1),x===0&&u===1&&(l+=1),x===1&&u===0&&(i+=1)});const c=e.length,r=(x,g)=>g>0?x/g:null,n=r(a,a+i),d=r(s,s+l),p=r(a,a+l),b=r(s,s+i),v=p!==null&&n!==null&&p+n>0?2*p*n/(p+n):null;return{umbral:o,verdaderosPositivos:a,verdaderosNegativos:s,falsosPositivos:l,falsosNegativos:i,exactitud:(a+s)/c,sensibilidad:n,especificidad:d,precision:p,valorPredictivoNegativo:b,f1:v,exactitudBalanceada:n!==null&&d!==null?(n+d)/2:null}}function Ee(e,t){const o=e.map((r,n)=>({observado:r,probabilidad:t[n]})).sort((r,n)=>n.probabilidad-r.probabilidad),a=e.reduce((r,n)=>r+n,0),s=e.length-a,i=[1/0,...new Set(o.map(r=>r.probabilidad)),-1/0].map(r=>{let n=0,d=0;return o.forEach(p=>{const b=p.probabilidad>=r;b&&p.observado===1&&(n+=1),b&&p.observado===0&&(d+=1)}),{umbral:Number.isFinite(r)?r:null,sensibilidad:a>0?n/a:0,unoMenosEspecificidad:s>0?d/s:0}}).sort((r,n)=>r.unoMenosEspecificidad-n.unoMenosEspecificidad||r.sensibilidad-n.sensibilidad);let c=0;for(let r=1;r<i.length;r+=1){const n=i[r].unoMenosEspecificidad-i[r-1].unoMenosEspecificidad,d=(i[r].sensibilidad+i[r-1].sensibilidad)/2;c+=n*d}return{puntos:i,auc:Math.max(0,Math.min(1,c))}}function Pe(e,t){const o=e.map((c,r)=>({observado:c,probabilidad:t[r]})).sort((c,r)=>c.probabilidad-r.probabilidad),a=Math.min(10,Math.max(3,Math.floor(e.length/5))),s=[];for(let c=0;c<a;c+=1){const r=Math.floor(c*o.length/a),n=Math.floor((c+1)*o.length/a),d=o.slice(r,n);if(!d.length)continue;const p=d.reduce((g,u)=>g+u.observado,0),b=d.reduce((g,u)=>g+u.probabilidad,0),v=d.length-p,x=d.length-b;s.push({grupo:s.length+1,n:d.length,probabilidadMedia:d.reduce((g,u)=>g+u.probabilidad,0)/d.length,observadosPositivos:p,esperadosPositivos:b,observadosNegativos:v,esperadosNegativos:x})}const l=s.reduce((c,r)=>{const n=r.esperadosPositivos>1e-12?(r.observadosPositivos-r.esperadosPositivos)**2/r.esperadosPositivos:0,d=r.esperadosNegativos>1e-12?(r.observadosNegativos-r.esperadosNegativos)**2/r.esperadosNegativos:0;return c+n+d},0),i=Math.max(s.length-2,1);return{estadistico:l,gradosLibertad:i,p:ie(l,i),grupos:s}}function Le(e,t,o,a){const s=e[0].length,l=e.map((n,d)=>{const p=S(a,n);return o[d]*(1-o[d])*A(n,p)}),i=t.map((n,d)=>{const p=Math.max(o[d]*(1-o[d]),1e-12);return(n-o[d])/Math.sqrt(p)}),c=t.map((n,d)=>{const p=Math.max(1e-12,Math.min(.999999999999,o[d])),b=n===1?2*Math.log(1/p):2*Math.log(1/(1-p));return Math.sign(n-p)*Math.sqrt(Math.max(b,0))}),r=i.map((n,d)=>{const p=Math.min(l[d],.999999999999);return n**2*p/(s*Math.max((1-p)**2,1e-12))});return{leverage:l,residuosPearson:i,residuosDeviance:c,distanciaCook:r}}function Re(e,t){return e.length<2?[]:e.map((o,a)=>{const s=e.filter((r,n)=>n!==a),l=se(s,!0),i=he(l),c=ge(i,l);try{const r=H(c),n=S(i,o),d=S(r,n),p=S(l,d),b=o.reduce(($,y)=>$+y,0)/o.length,v=o.reduce(($,y)=>$+(y-b)**2,0),x=o.reduce(($,y,E)=>$+(y-p[E])**2,0),g=v>1e-12?Math.max(0,1-x/v):1,u=Math.max(1-g,1e-12);return{variable:t[a],tolerancia:u,vif:1/u}}catch{return{variable:t[a],tolerancia:0,vif:1/0}}})}function Ie({y:e,X:t,nombresPredictores:o=[],incluirIntercepto:a=!0,nivelConfianza:s=.95,umbral:l=.5,maxIteraciones:i=100,tolerancia:c=1e-8}){const r=te("La variable dependiente",e,8);if(!r.every(f=>f===0||f===1))throw new Error("La variable dependiente debe estar codificada exclusivamente con 0 y 1.");const n=r.reduce((f,h)=>f+h,0),d=r.length-n;if(n===0||d===0)throw new Error("La variable dependiente debe contener observaciones de ambas categorías: 0 y 1.");if(!Array.isArray(t)||!t.length)throw new Error("Debe proporcionar al menos una variable predictora.");const p=t.map((f,h)=>te(o[h]||`X${h+1}`,f,8));if(!p.every(f=>f.length===r.length))throw new Error("Todas las variables deben contener la misma cantidad de observaciones.");if(!(s>0&&s<1))throw new Error("El nivel de confianza debe estar entre 0 y 1.");if(!(l>0&&l<1))throw new Error("El umbral de clasificación debe estar entre 0 y 1.");const b=p.map((f,h)=>o[h]||`X${h+1}`),v=se(p,a),x=r.length,g=v[0].length;if(x<=g+2)throw new Error("No hay suficientes observaciones para estimar el modelo logístico con esta cantidad de predictores.");const u=ke(v,r,{maxIteraciones:i,tolerancia:c});if(!u.convergio)throw new Error("El algoritmo no alcanzó convergencia. Revise separación perfecta, predictores extremos, colinealidad o tamaño muestral insuficiente.");const $=Me(r,a),y=p.length,E=Math.max(0,2*(u.logLik-$.logLik)),M=ie(E,y),I=we(.5+s/2),N=1-s,_=[...a?["Intercepto"]:[],...b],w=u.beta.map((f,h)=>{const j=Math.sqrt(Math.max(u.covarianza[h][h],0)),J=j>1e-12?f/j:f===0?0:1/0,me=$e(J),Q=f-I*j,ee=f+I*j;return{termino:_[h],estimacion:f,errorEstandar:j,z:J,p:me,intervaloConfianza:{inferior:Q,superior:ee,nivel:s},oddsRatio:Math.exp(Math.max(-35,Math.min(35,f))),intervaloOddsRatio:{inferior:Math.exp(Math.max(-35,Math.min(35,Q))),superior:Math.exp(Math.max(-35,Math.min(35,ee))),nivel:s}}}),C=Ce(r,u.probabilidades,l),q=Ee(r,u.probabilidades),L=Pe(r,u.probabilidades),P=Le(v,r,u.probabilidades,u.covarianza),O=Re(p,b),B=r.map((f,h)=>({observacion:h+1,observado:f,probabilidad:u.probabilidades[h],residuoPearson:P.residuosPearson[h],residuoDeviance:P.residuosDeviance[h],leverage:P.leverage[h],distanciaCook:P.distanciaCook[h]})).filter(f=>Math.abs(f.residuoPearson)>2||f.leverage>2*g/x||f.distanciaCook>4/x),re=-2*u.logLik,ne=-2*$.logLik,ce=-2*u.logLik+2*g,le=-2*u.logLik+g*Math.log(x),de=$.logLik!==0?1-u.logLik/$.logLik:null,K=1-Math.exp(2/x*($.logLik-u.logLik)),W=1-Math.exp(2/x*$.logLik),pe=W>1e-12?K/W:null,ue=C.exactitud===1,be=u.beta.some(f=>Math.abs(f)>15),Z=ue&&be,xe=[M!==null&&M<N?`El modelo global mejora significativamente al modelo nulo (χ² = ${E.toFixed(4)}, gl = ${y}, p ${M<.001?"< 0.001":`= ${M.toFixed(4)}`}).`:`El modelo global no alcanza significación estadística con α = ${N.toFixed(3)} (χ² = ${E.toFixed(4)}, gl = ${y}, p = ${M?.toFixed(4)??"no disponible"}).`,`El área bajo la curva ROC es ${q.auc.toFixed(4)}, lo que resume la capacidad del modelo para ordenar casos positivos por encima de casos negativos.`,`Con umbral ${l.toFixed(2)}, la exactitud es ${(C.exactitud*100).toFixed(2)} %, la sensibilidad es ${C.sensibilidad===null?"no calculable":`${(C.sensibilidad*100).toFixed(2)} %`} y la especificidad es ${C.especificidad===null?"no calculable":`${(C.especificidad*100).toFixed(2)} %`}.`,L.p!==null&&L.p>=N?`La prueba de Hosmer–Lemeshow no detecta una discrepancia significativa entre probabilidades observadas y esperadas (p = ${L.p.toFixed(4)}).`:`La prueba de Hosmer–Lemeshow sugiere revisar la calibración del modelo (p ${L.p!==null&&L.p<.001?"< 0.001":`= ${L.p?.toFixed(4)??"no disponible"}`}).`,O.some(f=>f.vif>=10)?"Se detectó multicolinealidad severa en al menos un predictor (VIF ≥ 10).":O.some(f=>f.vif>=5)?"Se detectó multicolinealidad moderada en al menos un predictor (VIF ≥ 5).":O.length?"No se detectó multicolinealidad relevante mediante el criterio VIF.":"El VIF no aplica a un modelo con un solo predictor.",B.length?`Se identificaron ${B.length} observaciones potencialmente influyentes que deben revisarse.`:"No se identificaron observaciones claramente influyentes mediante los criterios automáticos.",Z?"La clasificación perfecta junto con coeficientes extremos sugiere posible separación completa o cuasicompleta; los coeficientes pueden ser inestables.":"No se detectó el patrón automático de separación completa basado en clasificación perfecta y coeficientes extremos."];return{tipoModelo:"regresion-logistica-binaria",n:x,numeroPredictores:p.length,incluirIntercepto:a,nombresPredictores:b,nivelConfianza:s,alfa:N,convergencia:{convergio:u.convergio,iteraciones:u.iteraciones,posibleSeparacion:Z},distribucionRespuesta:{positivos:n,negativos:d,proporcionPositivos:n/x},coeficientes:w,ajuste:{logVerosimilitud:u.logLik,logVerosimilitudNula:$.logLik,deviance:re,devianceNula:ne,aic:ce,bic:le,r2McFadden:de,r2CoxSnell:K,r2Nagelkerke:pe},pruebaGlobal:{chiCuadrado:E,gradosLibertad:y,p:M},clasificacion:C,roc:q,calibracion:L,diagnosticos:{...P,vif:O,observacionesInfluyentes:B},probabilidades:u.probabilidades,datosGrafico:r.map((f,h)=>({observacion:h+1,observado:f,probabilidad:u.probabilidades[h],clasePredicha:u.probabilidades[h]>=l?1:0,residuoPearson:P.residuosPearson[h],residuoDeviance:P.residuosDeviance[h],leverage:P.leverage[h],distanciaCook:P.distanciaCook[h],predictores:p.map(j=>j[h])})),interpretacion:xe}}function je(e,t,o=e?.clasificacion?.umbral??.5){if(!e?.coeficientes?.length)throw new Error("El modelo proporcionado no es válido.");if(!Array.isArray(t)||t.length!==e.numeroPredictores)throw new Error(`Debe proporcionar ${e.numeroPredictores} valores predictores.`);if(!(o>0&&o<1))throw new Error("El umbral de clasificación debe estar entre 0 y 1.");const a=t.map(r=>r==null||typeof r=="string"&&r.trim()===""?NaN:Number(r));if(!a.every(Number.isFinite))throw new Error("Los valores predictores deben ser numéricos.");const s=e.coeficientes.map(r=>r.estimacion),l=[...e.incluirIntercepto?[1]:[],...a],i=A(l,s),c=G(i);return{predictorLineal:i,probabilidad:c,umbral:o,clasePredicha:c>=o?1:0}}function Y(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function k(e,t=3){return e==null?"—":Number.isFinite(e)?Number(e).toLocaleString("es-DO",{minimumFractionDigits:0,maximumFractionDigits:t}):e>0?"∞":"—"}function z(e,t,o,a=""){return`
    ...
    `}function Ne(e){const a={izquierda:62,derecha:24,superior:26,inferior:58},s=560-a.izquierda-a.derecha,l=390-a.superior-a.inferior,i=d=>a.izquierda+d*s,c=d=>a.superior+(1-d)*l,r=e.roc.puntos.map(d=>`${i(d.unoMenosEspecificidad)},${c(d.sensibilidad)}`).join(" "),n=`
    ...
    `;return z("Curva ROC","Relaciona sensibilidad y tasa de falsos positivos para todos los umbrales posibles. La diagonal representa una clasificación sin capacidad discriminativa.",n)}function ze(e){const t=e.clasificacion,o=(s,l,i)=>`
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-5 text-center">
            ${[["Exactitud",t.exactitud],["Sensibilidad",t.sensibilidad],["Especificidad",t.especificidad],["F1",t.f1]].map(([s,l])=>`
                <div class="rounded-xl bg-slate-50 border border-slate-200 px-3 py-4">
                    ...
                    <strong class="block mt-1 text-xl text-slate-900">${l===null?"—":`${k(l*100,2)} %`}</strong>
                </div>
            `).join("")}
        </div>
    `;return z("Matriz de confusión",`Resume la clasificación con umbral ${k(t.umbral,2)}. Los falsos positivos y falsos negativos deben interpretarse según el costo real de cada error.`,a)}function Se(e,t){const i=b=>48+b*504,n=e.datosGrafico.map((b,v)=>{const x=b.observado===1?90:210,g=(v%7-3)*5,u=b.observado===1?"text-indigo-700":"text-sky-500";return`
            <circle
                cx="${i(b.probabilidad)}"
                cy="${x+g}"
                r="5"
                ...
                class="${u}"
            >
                <title>Observación ${b.observacion}: Y=${b.observado}, p=${k(b.probabilidad,4)}</title>
            </circle>
        ...
        <div class="overflow-x-auto">
            <svg viewBox="0 0 600 300" class="min-w-[560px] w-full" role="img" aria-label="Distribución de probabilidades por clase observada">
                ...
                ${n}
                <text x="${600/2}" y="296" text-anchor="middle" font-size="13" font-weight="700" fill="currentColor" class="text-slate-700">Probabilidad estimada de ${Y(t)} = 1</text>
            </svg>
        </div>
    `;return z("Probabilidades estimadas por clase","Una buena separación visual concentra los casos observados con valor 1 hacia probabilidades altas y los casos con valor 0 hacia probabilidades bajas.",p)}function Ae(e){const t=e.calibracion.grupos.map(p=>({...p,proporcionObservada:p.n>0?p.observadosPositivos/p.n:0})),o=560,a=390,s={izquierda:62,derecha:24,superior:26,inferior:58},l=o-s.izquierda-s.derecha,i=a-s.superior-s.inferior,c=p=>s.izquierda+p*l,r=p=>s.superior+(1-p)*i,n=t.map(p=>`${c(p.probabilidadMedia)},${r(p.proporcionObservada)}`).join(" "),d=`
        <div class="overflow-x-auto">
    ...
    `;return z("Calibración","Compara la probabilidad media estimada con la proporción realmente observada en grupos ordenados por riesgo. Los puntos próximos a la diagonal indican mejor calibración.",d)}function Fe(e){const t=e.coeficientes.filter(x=>x.termino!=="Intercepto");if(!t.length)return"";const o=620,a=58,s=80+t.length*a,l=170,i=40,c=t.flatMap(x=>[Math.log(Math.max(x.intervaloOddsRatio.inferior,1e-8)),Math.log(Math.max(x.intervaloOddsRatio.superior,1e-8)),0]);let r=Math.min(...c),n=Math.max(...c);Math.abs(n-r)<.5&&(r-=.5,n+=.5);const d=n-r,p=x=>l+(Math.log(Math.max(x,1e-8))-r)/d*(o-l-i),b=p(1),v=`
        <div class="overflow-x-auto">
            <svg viewBox="0 0 ${o} ${s}" class="min-w-[580px] w-full" role="img" aria-label="Gráfico de razones de probabilidades">
                ...
                ${t.map((x,g)=>{const u=58+g*a,$=p(x.intervaloOddsRatio.inferior),y=p(x.intervaloOddsRatio.superior),E=p(x.oddsRatio),M=x.p<.05;return`
                        <text x="${l-14}" y="${u+4}" text-anchor="end" font-size="12" font-weight="700" fill="currentColor" class="text-slate-700">${Y(x.termino)}</text>
                        <line x1="${$}" y1="${u}" x2="${y}" y2="${u}" stroke="currentColor" stroke-width="4" class="${M?"text-indigo-500":"text-slate-400"}" />
                        <line x1="${$}" y1="${u-7}" x2="${$}" y2="${u+7}" stroke="currentColor" stroke-width="2" class="${M?"text-indigo-500":"text-slate-400"}" />
                        <line x1="${y}" y1="${u-7}" x2="${y}" y2="${u+7}" stroke="currentColor" stroke-width="2" class="${M?"text-indigo-500":"text-slate-400"}" />
                        <circle cx="${E}" cy="${u}" r="7" fill="currentColor" class="${M?"text-indigo-800":"text-slate-600"}">
                            <title>${Y(x.termino)}: OR=${k(x.oddsRatio,4)}, IC [${k(x.intervaloOddsRatio.inferior,4)}, ${k(x.intervaloOddsRatio.superior,4)}]</title>
                        </circle>
                    `}).join("")}

                <text x="${l}" y="${s-10}" text-anchor="start" font-size="11" fill="currentColor" class="text-slate-600">${k(Math.exp(r),3)}</text>
                <text x="${o-i}" y="${s-10}" text-anchor="end" font-size="11" fill="currentColor" class="text-slate-600">${k(Math.exp(n),3)}</text>
            </svg>
        </div>
    `;return z("Razones de probabilidades e intervalos de confianza","Valores mayores que 1 indican aumento de las odds del evento por unidad del predictor; valores menores que 1 indican disminución, manteniendo constantes los demás predictores.",v,"xl:col-span-2")}function qe(e){const t=e.datosGrafico.map(i=>i.distanciaCook),o=Math.max(...t,4/e.n,1e-8),a=4/e.n,s=e.datosGrafico.map(i=>{const c=Math.max(2,i.distanciaCook/o*180);return`
            <div class="flex flex-col justify-end items-center min-w-[18px] h-full">
                <div
                    ...
                    style="height:${c}px"
                    title="Observación ${i.observacion}: Cook = ${k(i.distanciaCook,5)}"
                ></div>
    ...
    `;return z("Distancia de Cook","Señala observaciones con potencial influencia sobre el ajuste. El criterio 4/n es orientativo y no sustituye la revisión del caso y del diseño del estudio.",l)}function Oe(e,t="Y"){return`
        <section class="mt-10">
    ...
    `}function tt(){const e=document.createElement("section");let t=null,o=null;e.className=`
        ...
        <section class="grid grid-cols-1 xl:grid-cols-[0.70fr_1.30fr] gap-7 mt-8">
            <aside class="rounded-3xl border border-slate-200 bg-white p-6 md:p-8 shadow-lg">
                ...
                <div class="space-y-5 text-slate-600 leading-relaxed">
                    ${D("Resultado binario","La primera columna debe contener exclusivamente 0 y 1. Defina con claridad qué representa el evento codificado como 1.")}
                    ${D("Predictores","Coloque después una columna por cada variable explicativa cuantitativa o previamente codificada.")}
                    ${D("Encabezados","La primera fila puede contener nombres de variables. Sin encabezados se asignarán Y, X1, X2, …")}
                    ${D("Interpretación","Una razón de probabilidades mayor que 1 indica aumento de las odds del evento; una menor que 1 indica disminución, manteniendo constantes los demás predictores.")}
                </div>
...
1, 7, 90"
        ...
        <section
            ...
            aria-live="polite"
        ></section>
    `;const a=e.querySelector("#formulario-regresion-logistica"),s=e.querySelector("#mensaje-error-logistica"),l=e.querySelector("#resultados-regresion-logistica");return a.elements.archivo.addEventListener("change",async()=>{const i=a.elements.archivo.files?.[0];if(i)try{a.elements.datos.value=await i.text(),V(s)}catch{oe(s,"No fue posible leer el archivo seleccionado.")}}),a.addEventListener("submit",i=>{i.preventDefault(),V(s);try{const{solicitud:c,metadatos:r}=Ve(a),n=Ie(c);t=n,o=r,l.innerHTML=We(n,r),l.classList.remove("hidden"),l.scrollIntoView({behavior:"smooth",block:"start"})}catch(c){l.classList.add("hidden"),t=null,o=null,oe(s,c instanceof Error?c.message:"No fue posible ajustar el modelo logístico.")}}),e.addEventListener("submit",i=>{const c=i.target.closest("#formulario-prediccion-logistica");if(!c||!t)return;i.preventDefault();const r=[...c.querySelectorAll("[data-predictor]")].map(d=>d.value.trim()===""?NaN:Number(d.value)),n=c.querySelector("#resultado-prediccion-logistica");try{const d=je(t,r,t.clasificacion.umbral);n.innerHTML=`
...
1, 7.5, 92
1, 8.0, 94
1, 8.5, 96`,e.elements.nivelConfianza.value="0.95",e.elements.umbral.value="0.50",e.elements.incluirIntercepto.checked=!0}function oe(e,t){e.textContent=t,e.classList.remove("hidden"),e.scrollIntoView({behavior:"smooth",block:"center"})}function V(e){e.textContent="",e.classList.add("hidden")}function R(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function m(e,t=4){return e==null?"—":Number.isFinite(e)?Number(e).toLocaleString("es-DO",{minimumFractionDigits:0,maximumFractionDigits:t}):e>0?"∞":"—"}function T(e){return e==null||!Number.isFinite(e)?"no disponible":e<.001?"< 0.001":`= ${m(e,4)}`}function U(e,t,o){return`
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    ${e.coeficientes.map(t=>`
                        <tr class="${t.p<(e.alfa??.05)?"bg-rose-50":""}">
                            <th scope="row" class="px-4 py-3 text-left font-black text-slate-900">${R(t.termino)}</th>
                            ...
                            <td class="px-4 py-3 text-right font-black">${T(t.p)}</td>
    ...
    `}function He(e){const t=e.clasificacion;return`
        <div class="overflow-x-auto rounded-2xl border border-slate-200">
            <table class="min-w-full text-sm">
                <caption class="sr-only">
    ...
    `}function Xe(e){return e.diagnosticos.vif.length?`
        <div class="overflow-x-auto rounded-2xl border border-slate-200">
            <table class="min-w-full text-sm">
                <caption class="sr-only">
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    ${e.diagnosticos.vif.map(t=>{const o=t.vif>=10?"Severa":t.vif>=5?"Moderada":"Sin evidencia relevante";return`
                            <tr>
                                <th scope="row" class="px-4 py-3 text-left font-black text-slate-900">${R(t.variable)}</th>
    ...
    `}function We(e,t){const o=e.pruebaGlobal.p!==null&&e.pruebaGlobal.p<(e.alfa??.05);return`
        ...
        <section class="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-7">
            ${U("χ² global",m(e.pruebaGlobal.chiCuadrado,4),`p ${T(e.pruebaGlobal.p)}`)}
            ${U("R² Nagelkerke",m(e.ajuste.r2Nagelkerke,4),"Pseudo-R² escalado entre 0 y 1")}
            ${U("AUC",m(e.roc.auc,4),"Capacidad discriminativa global")}
            ${U("Exactitud",`${m(e.clasificacion.exactitud*100,2)} %`,`Umbral ${m(e.clasificacion.umbral,2)}`)}
        </section>
    ...
    `}function Ze(e,t){const o=["REGRESIÓN LOGÍSTICA BINARIA — LABORATORIO KERNEL","",`Variable dependiente: ${t.nombreDependiente} (evento = 1)`,`Predictores: ${e.nombresPredictores.join(", ")}`,`n = ${e.n}`,`χ² global = ${m(e.pruebaGlobal.chiCuadrado,4)}`,`p del modelo ${T(e.pruebaGlobal.p)}`,`R² Nagelkerke = ${m(e.ajuste.r2Nagelkerke,4)}`,`AUC = ${m(e.roc.auc,4)}`,`Exactitud = ${m(e.clasificacion.exactitud*100,2)} %`,`Sensibilidad = ${e.clasificacion.sensibilidad===null?"no calculable":`${m(e.clasificacion.sensibilidad*100,2)} %`}`,`Especificidad = ${e.clasificacion.especificidad===null?"no calculable":`${m(e.clasificacion.especificidad*100,2)} %`}`,"","Coeficientes:"];return e.coeficientes.forEach(a=>{o.push(`${a.termino}: B = ${m(a.estimacion,6)}, OR = ${m(a.oddsRatio,5)}, z = ${m(a.z,4)}, p ${T(a.p)}`)}),o.push("","Interpretación:"),e.interpretacion.forEach(a=>o.push(`- ${a}`)),o.push("","Diseño y desarrollo de las calculadoras: Miguel Antonio Leonardo Sepúlveda y Natanael Ureña Castillo."),o.join(`
`)}function Je(e){return`"${String(e??"").replaceAll('"','""')}"`}function Qe(e,t){const a=[["REGRESIÓN LOGÍSTICA BINARIA"],["Variable dependiente",t.nombreDependiente],["Predictores",e.nombresPredictores.join(", ")],["n",e.n],["Chi-cuadrado global",e.pruebaGlobal.chiCuadrado],["p del modelo",e.pruebaGlobal.p],["R2 McFadden",e.ajuste.r2McFadden],["R2 Cox-Snell",e.ajuste.r2CoxSnell],["R2 Nagelkerke",e.ajuste.r2Nagelkerke],["AIC",e.ajuste.aic],["BIC",e.ajuste.bic],["AUC",e.roc.auc],["Umbral",e.clasificacion.umbral],["Exactitud",e.clasificacion.exactitud],["Sensibilidad",e.clasificacion.sensibilidad],["Especificidad",e.clasificacion.especificidad],[],["Término","Coeficiente","Error estándar","z","p","Odds ratio","IC OR inferior","IC OR superior"],...e.coeficientes.map(i=>[i.termino,i.estimacion,i.errorEstandar,i.z,i.p,i.oddsRatio,i.intervaloOddsRatio.inferior,i.intervaloOddsRatio.superior]),[],["Observación","Observado","Probabilidad","Clase predicha","Residuo Pearson","Residuo deviance","Leverage","Distancia de Cook"],...e.datosGrafico.map(i=>[i.observacion,i.observado,i.probabilidad,i.clasePredicha,i.residuoPearson,i.residuoDeviance,i.leverage,i.distanciaCook]),[],["Diseño y desarrollo","Miguel Antonio Leonardo Sepúlveda y Natanael Ureña Castillo"]].map(i=>i.map(Je).join(",")).join(`
`),s=new Blob(["\uFEFF",a],{type:"text/csv;charset=utf-8"}),l=document.createElement("a");l.href=URL.createObjectURL(s),l.download="regresion-logistica-binaria-kernel.csv",l.click(),URL.revokeObjectURL(l.href)}async function et(e,t,o){const a=Ze(e,t),s=o.textContent;try{await navigator.clipboard.writeText(a),o.textContent="Reporte copiado"}catch{const l=document.createElement("textarea");l.value=a,document.body.appendChild(l),l.select(),document.execCommand("copy"),l.remove(),o.textContent="Reporte copiado"}window.setTimeout(()=>{o.textContent=s},1800)}export{tt as CalculadoraRegresionLogistica};
