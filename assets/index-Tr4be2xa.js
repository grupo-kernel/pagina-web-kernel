...
>
  ...
  `}function fi(){const t=document.createElement("section");return t.className=`
    ...
    `}function Pi(t){const e=t.formacion||[],n=t.experiencia||[],a=e.length?e.map(h=>`
...
Gracias.`);return`
    ...
    `}function Mi(t=[]){return[...t].sort((e,n)=>e.featured&&!n.featured?-1:!e.featured&&n.featured?1:0)}function Fi(){const t=document.createElement("section");t.className=`
        ...
        tabletBig:w-7xl
        xl:w-full
    `;const e=Mi(_n),n=e.find(a=>a.featured)||e[0];return t.innerHTML=`
        ${$i(n)}
        ${zi(e)}
        ${Ui()}
    ...
    `}function Ji(t){if(!t)return`
        ...
        `;const e=Tn(t),n=Wi(t),a=n.length,o=n.map(r=>Number(r.year)).filter(Boolean),u=o.length?Math.min(...o):"",d=o.length?Math.max(...o):"",h=[...new Set(n.map(r=>r.q).filter(Boolean))],g=[...new Set(n.map(r=>Pa(r)).filter(Boolean))],l=n.map(r=>{const i=Pa(r),s=Rn(r),c=Gi(r);return`
...
};`,o.disabled=!0}function s(f,x="success"){d.textContent=f,d.classList.remove("hidden","border","border-emerald-200","bg-emerald-50","text-emerald-800","border-red-200","bg-red-50","text-red-800"),d.classList.add("border"),x==="error"?d.classList.add("border-red-200","bg-red-50","text-red-800"):d.classList.add("border-emerald-200","bg-emerald-50","text-emerald-800")}function c(f){const x=f.split(/\r?\n/).map(y=>y.trim()).filter(Boolean),w=[],b=[],v=new Set;return x.forEach((y,E)=>{const k=y.split(/[\t,; ]+/).map(R=>R.trim()).filter(Boolean),A=E+1,C=k[0],P=k.slice(1);if(!C){b.push(`Línea ${A}: falta la matrícula.`);return}if(!/^\d+$/.test(C)){b.push(`Línea ${A}: la matrícula "${C}" no es válida.`);return}if(v.has(C)){b.push(`Línea ${A}: la matrícula ${C} está repetida.`);return}if(P.length===0){b.push(`Línea ${A}: falta al menos una calificación.`);return}const I=P.map(R=>Number(R));if(I.some(R=>!Number.isFinite(R))){b.push(`Línea ${A}: existe una calificación no numérica.`);return}v.add(C),w.push({matricula:C,grades:I})}),{records:w,errors:b}}function p(f){return`var data_todo = {
...
if (!contenedor) {
  ...
  `;const n=e.querySelector("#kernel-login"),a=e.querySelector("#login-error"),o=e.querySelector("#correo"),u=e.querySelector("#password"),d=n.querySelector('button[type="submit"]');return n.addEventListener("submit",async h=>{h.preventDefault(),a.classList.add("hidden"),a.textContent="";const g=o.value.trim(),l=u.value;if(!g||!l){a.textContent="Introduzca el correo electrónico y la contraseña.",a.classList.remove("hidden");return}d.disabled=!0,d.textContent="Verificando...";try{await Id(g,l),t&&await t()}catch(r){console.error("Error de autenticación:",r);const i={"auth/invalid-email":"El correo electrónico no tiene un formato válido.","auth/invalid-credential":"El correo o la contraseña son incorrectos.","auth/user-not-found":"El correo no está autorizado.","auth/wrong-password":"El correo o la contraseña son incorrectos.","auth/user-disabled":"Este usuario está deshabilitado.","auth/too-many-requests":"Demasiados intentos. Espere unos minutos e inténtelo nuevamente.","auth/network-request-failed":"No fue posible conectarse con Firebase. Revise su conexión.","auth/unauthorized-domain":"El dominio de Grupo Kernel no está autorizado en Firebase."};a.textContent=i[r.code]||`No fue posible iniciar sesión. Error: ${r.code||"desconocido"}`,a.classList.remove("hidden")}finally{d.disabled=!1,d.textContent="Iniciar sesión"}}),e}function Rd(){return new Promise(t=>{const e=_d(n=>{e(),t(n)})})}async function Ld(){if(!await Rd())return Td(()=>{window.dispatchEvent(new HashChangeEvent("hashchange"))});const e=document.createElement("section");return e.className=`
        w-full max-w-7xl mx-auto
        px-4 py-10 md:px-8
        font-sans
    `,e.innerHTML=`
    ...
    `,e.querySelector("#cerrar-sesion-kernel")?.addEventListener("click",async()=>{await Pd(),window.location.reload()}),e}function jt(t,e,n,a,o,u){return`
        <article class="
            h-full bg-white
            border border-slate-200
            rounded-3xl p-7
