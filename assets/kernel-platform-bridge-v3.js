(() => {
  ...
  const parameter = key => urlState().searchParams.get(key) || hashParams().get(key);

  const isFormationRoute = () => (
    routeName().toLowerCase() === ROUTE.toLowerCase()
    && parameter(SECTION_KEY) === SECTION_VALUE
  );

  const selectedResearcherId = () => parameter(RESEARCHER_KEY) || "miguel-leonardo";
    ...
    try {
      ...
      main.innerHTML = `
        <section class="kernel-academic-v2" data-kernel-platform-page="academic-background-v2"
          ...
          <span hidden data-kernel-platform-page="academic-background"></span>
