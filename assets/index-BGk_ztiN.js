...
>
    ...
    `}function Ho(e,a){e.addEventListener("click",r=>{const t=r.target.closest(["[data-route]"]);t&&(r.preventDefault(),a(t.dataset.route))})}function Bl(e){Ho(e,ht)}const Wo=""+new URL("apec-ConmXkrT.png",import.meta.url).href,Gl=""+new URL("isfodosu-C3sCRZT7.png",import.meta.url).href,Hl=""+new URL("uasd-D9oiqsNN.png",import.meta.url).href,li=[{content:`<div class="relative flex flex-col justify-center items-center w-full h-full">
...
Gracias.`);return`
    ...
    `}function Jc(e){if(!e)return`
        ...
        `;const a=es(e),r=Kc(e),t=r.length,n=r.map(i=>Number(i.year)).filter(Boolean),c=n.length?Math.min(...n):"",s=n.length?Math.max(...n):"",p=[...new Set(r.map(i=>i.q).filter(Boolean))],u=[...new Set(r.map(i=>ci(i)).filter(Boolean))],l=r.map(i=>{const o=ci(i),d=as(i),m=Wc(i);return`
...
17`,l(),Yr(r),t.classList.add("hidden");return}d==="limpiar"&&(a.reset(),n.value="",c.value="",l(),Yr(r),t.innerHTML="",t.classList.add("hidden"))}),e}function Kr(e,a){return`
    ...
    `}function so(e,a){const r=e.trim();if(!r)throw new Error(`${a} no contiene datos.`);const t=r.split(/[\s;]+/).filter(Boolean),n=t.map(s=>Number(s.replace(",","."))),c=n.findIndex(s=>!Number.isFinite(s));if(c!==-1)throw new Error(`${a} contiene un valor no válido: "${t[c]}".`);if(n.length<2)throw new Error(`${a} debe contener al menos dos observaciones.`);return n}function lo(e){return e.trim()?e.trim().split(/[\s;]+/).filter(Boolean).length:0}function co(e){return e===1?"1 valor":`${e} valores`}function Sb(e,a){e.textContent=a,e.classList.remove("hidden")}function Yr(e){e.textContent="",e.classList.add("hidden")}function U(e,a=4){return Number.isFinite(e)?new Intl.NumberFormat("es-DO",{minimumFractionDigits:0,maximumFractionDigits:a}).format(e):"—"}function _b(e){return Number.isFinite(e)?e<.001?"< 0.001":U(e,4):"—"}function zb(e,a=.95){const r=1-a,t=e.valorP<r,n=r.toFixed(2),c=Math.round(a*100),s=e.id==="t-student-relacionadas"?e.diferenciaMedia:e.diferencias.mediana,p=Db(s),u=t?`Se observa evidencia estadísticamente significativa de una diferencia entre las dos mediciones al nivel α = ${n}, correspondiente a un nivel de confianza del ${c} %. ${p}`:`No se observa evidencia estadísticamente significativa de una diferencia entre las dos mediciones al nivel α = ${n}, correspondiente a un nivel de confianza del ${c} %. ${p}`;return`
        <section class="rounded-3xl border border-violet-200 bg-white shadow-xl overflow-hidden">
            ...
            <div class="px-6 py-8 md:px-10">
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                    ...
                    ${It("p",_b(e.valorP),"Valor bilateral")}
...
20`];[...a.querySelectorAll("[data-medicion]")].forEach((c,s)=>{c.querySelector('[data-campo="nombre"]').value=r[s],c.querySelector('[data-campo="valores"]').value=t[s],yl(c)})}function yl(e){const a=e.querySelector('[data-campo="valores"]'),r=e.querySelector("[data-contador]"),t=a.value.trim()?a.value.trim().split(/[\s;]+/).filter(Boolean).length:0;r.textContent=t===1?"1 valor":`${t} valores`}document.addEventListener("input",e=>{const a=e.target.closest('#formulario-mediciones-relacionadas [data-campo="valores"]');if(!a)return;const r=a.closest("[data-medicion]");r&&yl(r)});function bo(e,a){e.textContent=a,e.classList.remove("hidden")}function en(e){e.textContent="",e.classList.add("hidden")}function Ie(e,a=4){return Number.isFinite(e)?new Intl.NumberFormat("es-DO",{minimumFractionDigits:0,maximumFractionDigits:a}).format(e):e===1/0?"∞":"—"}function rr(e){return Number.isFinite(e)?e<.001?"< 0.001":Ie(e,4):"—"}function Tg(e){const a=e.id==="anova-medidas-repetidas",r=a?e.esfericidad.greenhouseGeisser.valorP:e.valorP,t=r<e.alfa,n=Math.round(e.nivelConfianza*100),c=a?"Greenhouse–Geisser":"sin corrección adicional",s=t?`Se observa evidencia estadísticamente significativa de diferencias entre las mediciones al nivel α = ${e.alfa.toFixed(2)}. Para la decisión principal se utilizó ${c}.`:`No se observa evidencia estadísticamente significativa de diferencias entre las mediciones al nivel α = ${e.alfa.toFixed(2)}. Para la decisión principal se utilizó ${c}.`;return`
    ...
    `}function zg(e){return e?.comparaciones?.length?`
...
20, 30, 7`}}[e];return`
    ...
    `}function Q0(e){return e.trim().split(/[\s,;]+/).filter(Boolean).map(Number)}function Ro(e,a){const r=e.trim().split(/\n+/).map(t=>t.trim()).filter(Boolean).map((t,n)=>{const c=t.split(/[,;\t ]+/).filter(Boolean).map(Number);if(c.length!==a||!c.every(Number.isFinite))throw new Error(`La fila ${n+1} debe contener exactamente ${a} valores numéricos.`);return c});if(!r.length)throw new Error("Introduzca los datos antes de calcular.");return r}function X0(e,a){const r=a.querySelector("textarea").value;if(!r.trim())throw new Error("Introduzca los datos antes de calcular.");if(e==="individuales")return{tipoDatos:e,valores:Q0(r)};if(e==="frecuencias"){const n=Ro(r,2);return{tipoDatos:e,valores:n.map(c=>c[0]),frecuencias:n.map(c=>c[1])}}const t=Ro(r,3);return{tipoDatos:e,clases:t.map(n=>({limiteInferior:n[0],limiteSuperior:n[1],frecuencia:n[2]}))}}function Z0(e,a){const r={individuales:"42, 48, 51, 53, 55, 55, 57, 59, 61, 64, 67, 72, 96",frecuencias:`1, 2
2, 5
3, 8
4, 6
...
`),n=new Blob(["\uFEFF",t],{type:"text/csv;charset=utf-8"}),c=document.createElement("a");c.href=URL.createObjectURL(n),c.download="regresion-logistica-binaria-kernel.csv",c.click(),URL.revokeObjectURL(c.href)}async function Mx(e,a,r){const t=Px(e,a),n=r.textContent;try{await navigator.clipboard.writeText(t),r.textContent="Reporte copiado"}catch{const c=document.createElement("textarea");c.value=t,document.body.appendChild(c),c.select(),document.execCommand("copy"),c.remove(),r.textContent="Reporte copiado"}window.setTimeout(()=>{r.textContent=n},1800)}async function Tx(){const e=await Mn(()=>import("./CalculadoraRegresionConteo-VE7dPXsR.js"),[],import.meta.url);if(typeof e.CalculadoraRegresionConteo!="function")throw new Error("La calculadora de regresión de conteo no está disponible.");return e.CalculadoraRegresionConteo()}async function Nx(){const e=await Mn(()=>import("./CalculadoraFiabilidadCuestionarios-cvzxa8nN.js"),[],import.meta.url);if(typeof e.CalculadoraFiabilidadCuestionarios!="function")throw new Error("La calculadora de cuestionarios y fiabilidad no está disponible.");return e.CalculadoraFiabilidadCuestionarios()}async function Sx(){const e=await Mn(()=>import("./CalculadoraEvaluacionEducativa-PuzJmWyR.js"),[],import.meta.url);if(typeof e.CalculadoraEvaluacionEducativa!="function")throw new Error("La calculadora de evaluación educativa no está disponible.");return e.CalculadoraEvaluacionEducativa()}const _x={home:{page:ec,layout:"default",title:"Portada | El Kernel"},quienesSomos:{page:vc,layout:"full",title:"Quiénes somos | El Kernel"},equipment:{page:Ac,layout:"default",title:"Equipo de investigación | El Kernel"},FormacionAcademica:{page:Mc,layout:"default",title:"Formación académica | El Kernel"},noticias:{page:Hc,layout:"full",title:"Noticias | El Kernel"},publicaciones:{page:Xc,layout:"default",title:"Publicaciones | El Kernel"},proyectos:{page:md,layout:"default",title:"Proyectos | El Kernel"},lineas:{page:od,layout:"default",title:"Líneas de investigación | El Kernel"},contacto:{page:Ed,layout:"default",title:"Contacto | El Kernel"},herramientas:{page:Ld,layout:"default",title:"Herramientas | El Kernel"},laboratorioKernel:{page:Nf,layout:"default",title:"Laboratorio Inteligente de Investigación | El Kernel"},asistentePruebas:{page:Wf,layout:"default",title:"¿Qué prueba debo utilizar? | El Kernel"},comparacionGrupos:{page:ob,layout:"default",title:"Comparación de grupos y mediciones | El Kernel"},calculadoraDosGrupos:{page:yb,layout:"default",title:"Comparación de dos grupos | El Kernel"},calculadoraDosMuestrasRelacionadas:{page:Nb,layout:"default",title:"Comparación de dos mediciones relacionadas | El Kernel"},calculadoraTresOMasGrupos:{page:ag,layout:"default",title:"Comparación de tres o más grupos independientes | El Kernel"},calculadoraTresOMasMedicionesRelacionadas:{page:Pg,layout:"default",title:"Comparación de tres o más mediciones relacionadas | El Kernel"},correlacionAsociacion:{page:Dg,layout:"default",title:"Correlación y asociación | El Kernel"},calculadoraRelacionVariables:{page:Kg,layout:"default",title:"Relación entre variables | El Kernel"},calculadoraAsociacionCategorica:{page:h0,layout:"default",title:"Asociación entre variables categóricas | El Kernel"},calculadoraEstadisticaDescriptiva:{page:Y0,layout:"default",title:"Estadística descriptiva | El Kernel"},calculadoraFiabilidadCuestionarios:{page:Nx,layout:"default",title:"Cuestionarios y fiabilidad | El Kernel"},calculadoraEvaluacionEducativa:{page:Sx,layout:"default",title:"Evaluación educativa | El Kernel"},regresionModelos:{page:ch,layout:"default",title:"Modelos de regresión | El Kernel"},calculadoraRegresion:{page:Mh,layout:"default",title:"Regresión lineal y múltiple | El Kernel"},calculadoraRegresionLogistica:{page:bx,layout:"default",title:"Regresión logística binaria | El Kernel"},calculadoraRegresionConteo:{page:Tx,layout:"default",title:"Regresión de Poisson y binomial negativa | El Kernel"}};let Oo=document.referrer||"";function ht(e){window.location.hash=`/${e}`}function zx(){const e=()=>{const a=window.location.hash.replace("#/","")||"home";Ox(a)};window.addEventListener("hashchange",e),e()}function Dx(e,a){const r=`${window.location.origin}${window.location.pathname}#/${e}`;typeof window.gtag=="function"&&window.gtag("event","page_view",{page_title:a,page_location:r,page_referrer:Oo}),Oo=r}function qx(e){const a=document.createElement("section");a.className=`
    ...
    `;const r=a.querySelector("[data-mensaje-error-ruta]");return r&&(r.textContent=e instanceof Error?e.message:"Se produjo un error inesperado."),a.querySelector("[data-action='reintentar-ruta']")?.addEventListener("click",()=>{window.location.reload()}),a.querySelector("[data-action='volver-laboratorio']")?.addEventListener("click",()=>{ht("laboratorioKernel")}),a}async function Ox(e){const a=document.querySelector("main"),r=_x[e];if(a){if(a.innerHTML="",!r){ht("home");return}Ic(r.layout),document.title=r.title;try{const t=await r.page();if(!(t instanceof Element))throw new Error("La herramienta no devolvió un componente válido.");a.appendChild(t),window.scrollTo({top:0,behavior:"auto"}),Dx(e,r.title)}catch(t){console.error(`[Kernel] Error al cargar la ruta ${e}.`,t),a.appendChild(qx(t))}}}function Fx(e,a){Ho(e,ht),zx();const r=window.matchMedia("(max-width: 768px)");let t=!1;function n(s){s.matches&&!t&&(oi(e),t=!0)}r.addEventListener("change",n),n(r),oi(e);const c=a.querySelector('[data-action ="open-menu"]');zl(c,e),Dl(e)}function Ux(e){const a=String(e??"").replace(/\u00a0/g,"").replace(/\s/g,"").replace(/[^0-9+\-.,eE]/g,"");if(!a)return null;const r=a.lastIndexOf(","),t=a.lastIndexOf(".");let n=a;r>=0&&t>=0?n=t>r?a.replace(/,/g,""):a.replace(/\./g,"").replace(",","."):r>=0&&(n=a.length-r-1===3&&r>0?a.replace(/,/g,""):a.replace(",","."));const c=Number(n);return Number.isFinite(c)?c:null}function jx(e){const a=e.querySelector("table");if(!a)return null;const t=[...a.querySelectorAll("thead th")].map(c=>c.textContent.trim().toLowerCase()).some(c=>c.includes("marca")),n=[...a.querySelectorAll("tbody tr")].map(c=>{const s=[...c.querySelectorAll("td")],p=Number(s[t?2:1]?.textContent.trim()),u=Number(s[t?4:3]?.textContent.trim());return{etiqueta:s[0]?.textContent.trim()||"",valor:Ux(s[t?1:0]?.textContent),frecuencia:Number.isFinite(p)?p:0,acumulada:Number.isFinite(u)?u:0}}).filter(c=>c.etiqueta&&c.frecuencia>=0);return{agrupados:t,filas:n}}function Tl(e){return Number.isFinite(Number(e))?Number(e).toLocaleString("es-DO",{maximumFractionDigits:3}):"—"}function Fo(e,a){let r=0;for(const t of e)if(r+=t.frecuencia,a<r)return t.valor;return e.at(-1)?.valor??null}function pn(e,a){const r=e.reduce((u,l)=>u+l.frecuencia,0);if(!r)return null;if(a<=0)return e[0].valor;if(a>=1)return e.at(-1).valor;const t=(r-1)*a,n=Math.floor(t),c=Math.ceil(t),s=Fo(e,n),p=Fo(e,c);return n===c?s:s*(c-t)+p*(t-n)}function Ja(e,a,r){return`
        <text
            x="${e}"
            y="${a}"
            text-anchor="middle"
        ...
        >${Tl(r)}</text>
    `}function Vx(e,a){if(!e||e.dataset.cajaDescriptivaSegura==="true"||a.agrupados)return;e.dataset.cajaDescriptivaSegura="true";const r=a.filas.filter(h=>h.valor!==null&&h.frecuencia>0).sort((h,w)=>h.valor-w.valor);if(r.reduce((h,w)=>h+w.frecuencia,0)<2)return;const n=r[0].valor,c=r.at(-1).valor,s=pn(r,.25),p=pn(r,.5),u=pn(r,.75),l=u-s,i=s-1.5*l,o=u+1.5*l,d=r.filter(h=>h.valor>=i&&h.valor<=o),m=r.filter(h=>h.valor<i||h.valor>o),f=d[0]?.valor??n,g=d.at(-1)?.valor??c,b=Math.max(c-n,1),x=h=>40+(h-n)/b*520,y=e.querySelector("svg");if(!y)return;y.setAttribute("aria-label","Diagrama de caja con cuartiles, mediana, bigotes y valores atípicos potenciales"),y.innerHTML=`
        ...
        ${Ja(x(u),30,u)}
        ${Ja(x(g),140,g)}
    `;const v=e.querySelector("h3 + p");v&&(v.textContent="Los bigotes terminan en los valores extremos no atípicos. Los puntos rojos representan valores atípicos potenciales según el criterio de Tukey.")}function Bx(){const e=document.createElement("section");return e.dataset.encabezadoGraficosSeguro="true",e.className="mt-10",e.innerHTML=`
        <p class="uppercase tracking-widest text-sky-700 text-xs font-black mb-2">
            Visualización estadística
        </p>
    ...
    `,e}function Gx(e){const a=document.createElement("article");a.className="rounded-3xl border border-slate-200 bg-white p-6 shadow-md overflow-hidden";const r=Math.max(...e.map(t=>t.frecuencia),1);return a.innerHTML=`
    ...
    `,a}function Wx(e){const a=[...e.querySelectorAll("article")],r=a.find(s=>s.querySelector("h3")?.textContent.includes("Distribución de frecuencias")),t=a.find(s=>s.querySelector("h3")?.textContent.includes("Diagrama de caja"));if(!r||!t)return;const n=r.parentElement,c=jx(e);if(!(!n||!c?.filas.length)&&(e.querySelector("[data-encabezado-graficos-seguro='true']")||(n.before(Bx()),n.classList.remove("mt-8")),Vx(t,c),!e.querySelector("[data-graficos-descriptivos-seguros='true']"))){const s=document.createElement("div");s.dataset.graficosDescriptivosSeguros="true",s.className="grid grid-cols-1 xl:grid-cols-2 gap-7 mt-7",s.append(Gx(c.filas),Hx(c.filas)),n.after(s)}}function Kx(){let e=!1;const a=()=>{e=!1;const n=document.querySelector("#resultados-estadistica-descriptiva");n&&!n.classList.contains("hidden")&&Wx(n)},r=()=>{e||(e=!0,window.requestAnimationFrame(a))};new MutationObserver(r).observe(document.body,{childList:!0,subtree:!0}),r()}function Yx(e,a){return`${e.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")||`grafico-${a+1}`}.svg`}function Jx(e,a,r){const t=e.cloneNode(!0);t.getAttribute("xmlns")||t.setAttribute("xmlns","http://www.w3.org/2000/svg");const c=new XMLSerializer().serializeToString(t),s=new Blob([c],{type:"image/svg+xml;charset=utf-8"}),p=URL.createObjectURL(s),u=document.createElement("a");u.href=p,u.download=Yx(a,r),document.body.appendChild(u),u.click(),u.remove(),setTimeout(()=>URL.revokeObjectURL(p),1e3)}function Qx(e,a,r){if(e.querySelector("[data-descargar-svg]"))return;const t=e.querySelector("h3")?.textContent.trim()||`Gráfico ${r+1}`,n=document.createElement("button");n.type="button",n.dataset.descargarSvg="true",n.className=`
    ...
    `,n.innerHTML=`
        <span aria-hidden="true">↓</span>
        Descargar SVG
    `,n.setAttribute("aria-label",`Descargar ${t} en formato SVG`),n.addEventListener("click",()=>{Jx(a,t,r)}),e.appendChild(n)}function Xx(e){const a=window.open("","_blank");if(!a){window.print();return}const r=`
        <style>
    ...
    `),a.document.close(),a.addEventListener("load",()=>{a.focus(),a.print()})}function Zx(e){if(e.querySelector("[data-herramientas-exportacion='true']"))return;const a=document.createElement("section");a.dataset.herramientasExportacion="true",a.className=`
    ...
    `,a.querySelector("[data-imprimir-informe]")?.addEventListener("click",()=>{Xx(e)}),e.prepend(a)}function ev(e){Zx(e),[...e.querySelectorAll("article")].forEach((r,t)=>{const n=r.querySelector("svg");n&&Qx(r,n,t)})}function av(){const e=()=>{const r=document.querySelector("#resultados-estadistica-descriptiva");r&&!r.classList.contains("hidden")&&ev(r)};new MutationObserver(e).observe(document.body,{childList:!0,subtree:!0}),e()}const tv=["#formulario-estadistica-descriptiva","#formulario-regresion","#formulario-regresion-conteo","[id*='calculadora'] form","section form"].join(",");function rv(){return document.querySelector("main h1, #app h1, h1")?.textContent?.trim()?.toLowerCase()||""}function nv(){return rv().includes("laboratorio inteligente de investigación")}function iv(){const e=document.querySelector("main h1, #app h1, h1"),a=document.querySelector("[data-action='volver-laboratorio'], [data-action='volver-modelos'], [data-action='volver-regresion']"),r=document.querySelector(tv);return!!(e&&a&&r)}function ov(){const e=document.createElement("section");return e.dataset.creditosCalculadoras="true",e.className=["w-full max-w-7xl mx-auto","px-4 pb-10 md:px-8","font-sans"].join(" "),e.innerHTML=`
    ...
    `,e}function Uo(){if(!iv()&&!nv()||document.querySelector("[data-creditos-calculadoras='true']"))return;const e=document.querySelector("main, #app, [data-router-view]");e&&e.appendChild(ov())}function sv(){new MutationObserver(Uo).observe(document.body,{childList:!0,subtree:!0}),Uo()}function lv(){return[...document.querySelectorAll("main h1")].find(a=>a.textContent.trim().toLowerCase().includes("laboratorio inteligente de investigación"))&&[...document.querySelectorAll("main article")].find(a=>a.querySelector("h3")?.textContent?.trim()?.toLowerCase()==="regresión")||null}function jo(){const e=lv();if(!e||e.dataset.regresionConectada==="true")return;const a=e.querySelector("h3 + p"),r=e.querySelector("button");a&&(a.textContent="Regresión lineal simple y múltiple, y regresión logística binaria, con inferencia, diagnósticos gráficos, predicción e interpretación guiada."),r&&(r.onclick=()=>{window.location.hash="/regresionModelos"},r.setAttribute("aria-label","Explorar los modelos de regresión")),e.dataset.regresionConectada="true"}function cv(){new MutationObserver(jo).observe(document.body,{childList:!0,subtree:!0}),jo()}const dv={"regresión lineal simple":{ruta:"calculadoraRegresion",tipo:"simple",almacenamiento:"kernel-tipo-modelo-regresion",clase:"rose",texto:"Abra la calculadora de regresión lineal y cargue una variable dependiente cuantitativa y un predictor."},"regresión lineal múltiple":{ruta:"calculadoraRegresion",tipo:"multiple",almacenamiento:"kernel-tipo-modelo-regresion",clase:"rose",texto:"Abra la calculadora de regresión lineal y cargue una variable dependiente cuantitativa y dos o más predictores."},"regresión logística binaria simple":{ruta:"calculadoraRegresionLogistica",tipo:"simple",almacenamiento:"kernel-tipo-modelo-regresion",clase:"rose",texto:"Abra la calculadora logística y cargue una variable dependiente codificada 0/1 y un predictor."},"regresión logística binaria múltiple":{ruta:"calculadoraRegresionLogistica",tipo:"multiple",almacenamiento:"kernel-tipo-modelo-regresion",clase:"rose",texto:"Abra la calculadora logística y cargue una variable dependiente codificada 0/1 y dos o más predictores."},"regresión de poisson":{ruta:"calculadoraRegresionConteo",tipo:"poisson",almacenamiento:"kernel-modelo-conteo",clase:"amber",texto:"Abra la calculadora de conteo y ajuste un modelo de Poisson con una variable dependiente formada por enteros no negativos."},"regresión binomial negativa":{ruta:"calculadoraRegresionConteo",tipo:"negativa",almacenamiento:"kernel-modelo-conteo",clase:"amber",texto:"Abra la calculadora de conteo y ajuste un modelo binomial negativo cuando exista sobredispersión respecto a Poisson."}};function uv(){const e=[...document.querySelectorAll("main h1")];for(const a of e){const r=a.textContent.trim().toLowerCase(),t=dv[r];if(t)return{encabezado:a,modelo:t}}return null}function pv(e){const a=document.createElement("div"),r=e.clase==="amber",t=r?"border-amber-200":"border-rose-200",n=r?"bg-amber-50":"bg-rose-50",c=r?"text-amber-700":"text-rose-700",s=r?"bg-amber-600 hover:bg-amber-700":"bg-rose-700 hover:bg-rose-800";return a.dataset.ejecutarModeloRegresion="true",a.className=`mt-8 rounded-2xl border ${t} ${n} p-6`,a.innerHTML=`
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
            <div>
    ...
    `,a.querySelector("button")?.addEventListener("click",p=>{const u=p.currentTarget,l=u.dataset.almacenamientoRegresion||"kernel-tipo-modelo-regresion";sessionStorage.setItem(l,u.dataset.tipoRegresion||""),window.location.hash=`/${u.dataset.routeRegresion}`}),a}function Vo(){const e=uv();if(!e)return;const a=e.encabezado.closest("section");if(!a||a.querySelector("[data-ejecutar-modelo-regresion='true']"))return;(a.querySelector("header + div")||a).appendChild(pv(e.modelo))}function mv(){new MutationObserver(Vo).observe(document.body,{childList:!0,subtree:!0}),Vo()}function fv(){return[...document.querySelectorAll("main h1")].find(a=>a.textContent.trim().toLowerCase().includes("laboratorio inteligente de investigación"))&&[...document.querySelectorAll("main article")].find(a=>a.querySelector("h3")?.textContent?.trim()?.toLowerCase()==="cuestionarios y fiabilidad")||null}function Bo(){const e=fv();if(!e||e.dataset.fiabilidadConectada==="true")return;const a=e.querySelector("h3 + p"),r=e.querySelector("button");a&&(a.textContent="Alfa de Cronbach, alfa estandarizado, omega aproximado, análisis de ítems, recodificación y gráficos diagnósticos."),r&&(r.onclick=()=>{window.location.hash="/calculadoraFiabilidadCuestionarios"},r.setAttribute("aria-label","Abrir la calculadora de cuestionarios y fiabilidad")),e.dataset.fiabilidadConectada="true"}function bv(){new MutationObserver(Bo).observe(document.body,{childList:!0,subtree:!0}),Bo()}function gv(e){e.target.closest("#asistente-pruebas [data-value='instrumento']")&&(e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation(),window.location.hash="/calculadoraFiabilidadCuestionarios")}function hv(){document.addEventListener("click",gv,!0)}function la(e,a){try{a()}catch(r){console.error(`[Kernel] No fue posible iniciar ${e}.`,r)}}const cr=document.querySelector("#header"),dr=document.querySelector("#navBar"),Go=document.querySelector("#footer");cr&&(cr.innerHTML=Nl());dr&&(dr.innerHTML=Sl());Go&&(Go.innerHTML=_l());la("la navegación",()=>{dr&&cr&&Fx(dr,cr)});la("los gráficos de estadística descriptiva",Kx);la("la exportación de estadística descriptiva",av);la("los créditos de las calculadoras",sv);la("la integración del área de regresión",cv);la("la integración de regresión con el asistente",mv);la("la integración de cuestionarios y fiabilidad",bv);la("cuestionarios y fiabilidad en el asistente",hv);
