(() => {
  ...
  function replaceInstitutionStrip() {
    ...
    if (current.join("|") === MIGUEL_AFFILIATIONS.join("|")) return;
    ...
    MIGUEL_AFFILIATIONS.forEach((institution, index) => {
      const item = document.createElement(first.tagName === "A" ? "span" : first.tagName.toLowerCase());
      item.className = first.className;
      item.textContent = institution;
      item.dataset.kernelInstitution = institution;
      marker.parentNode?.insertBefore(item, marker.nextSibling);
