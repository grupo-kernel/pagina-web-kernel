(() => {
  ...
  function descriptor() {
    ...
    if (route === "home") {
      return { key: "home", marker: '[data-kernel-platform-page="home-2b"]' };
    }
    if (route === "equipment") {
      return { key: "equipment", marker: '[data-kernel-platform-page="team-nine"]' };
    }
    if (route === "quienessomos" && params.get("kernelSection") === "formacion") {
      return { key: "formation", marker: '[data-kernel-platform-page="academic-background"]' };
    }
    if (route === "publicaciones") {
      return { key: "publications", marker: '[data-kernel-platform-page="publications-2"]' };
    }
    if (route === "proyectos") {
      return { key: "projects", marker: '[data-kernel-platform-page="projects-2"]' };
    }
    if (route === "laboratoriokernel") {
      return { key: "laboratory", laboratory: true };
    }
    return null;
  }
  ...
  function installStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      html[data-kernel-route-pending="true"] #main{
      ...
      html[data-kernel-route-pending="true"] #main::before{
        content:var(--kernel-loading-label,"Cargando la plataforma…");
        position:absolute;
        inset:0;
        z-index:40;
        display:grid;
        ...
        color:#0f5b5d;
        font:900 clamp(.9rem,2vw,1.12rem)/1.4 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;
        text-align:center;
        letter-spacing:.01em;
      }
      html[data-kernel-route-pending="true"] #main::after{
        content:"";
        position:absolute;
        left:50%;
        top:calc(50% - 2.6rem);
        ...
        border:.22rem solid rgba(15,91,93,.18);
        border-top-color:#0f5b5d;
        border-radius:50%;
        animation:kernel-stability-spin .78s linear infinite;
      }
      html[data-kernel-route-pending="true"] #footer{visibility:hidden}
      html[data-kernel-route-pending="false"] #main>*{
        animation:kernel-stability-reveal .18s ease-out both;
      }
      ...
      @media(prefers-reduced-motion:reduce){
        html[data-kernel-route-pending="true"] #main::after{animation:none}
        html[data-kernel-route-pending="false"] #main>*{animation:none}
      }
    `;
  ...
  function setLoadingLabel() {
    const label = language() === "en" ? "Loading the integrated platform…" : "Cargando la plataforma integrada…";
  ...
  function laboratoryReady(main) {
    ...
    return /(Iniciar sesión|Acceso|Authentication|Sign in|Asistente|Assistant|Calculadora|Calculator|Biblioteca|Library)/i.test(text);
  }
